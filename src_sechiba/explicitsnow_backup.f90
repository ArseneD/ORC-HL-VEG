...
!
          ...
          zckt_ice(ji,:) = (zc1_ice/ztemp(ji,:)**2)*(zexpo(ji,:) - 1.0)*EXP(-1.0 * zexpo(ji,:))
          !
          zexpo(ji,:)    = chalev0/(xrv*ztemp(ji,:))
          zckt_liq(ji,:) = (zc1_liq/ztemp(ji,:)**2)*(zexpo(ji,:) - 1.0)*EXP(-1.0 * zexpo(ji,:))
          !
...
!!!!!!!!!!!!!!!write(1,*) 'tao debug !!! in explicitsnowmelt,snow',SUM(snowdz(ji,:)*snowrho(ji,:))
...
!!!!!!!!!               write(1,*) 'tao debug in explicitsnowmelt,snowmelt,zflowliqt,zpcpxs,snow',snowmelt(ji),zflowliqt(nsnow),zpcpxs,SUM(snowdz(ji,:)*snowrho(ji,:))
