...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_sechiba/hydrolc.f90 $
...
MODULE hydrolc
  ...
  LOGICAL, SAVE                             :: l_first_hydrol=.TRUE. !! Initialisation has to be done one time (true/false)
...
!! If we choose to check the water balance, this is done over each timestep dtradia, 
!! by hydrolc_waterbal. 
!!
...
!! dtradia, with a unit of \f$kg.m^{-2}\f$.
!!
!! RECENT CHANGE(S) : None
!! 
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), INTENT (in)                                :: dtradia          !! Time step duration (s)  
    LOGICAL, INTENT(in)                                     :: ldrestart_read   !! Logical to read  _restart_ file (true/false)
    LOGICAL, INTENT(in)                                     :: ldrestart_write  !! Logical to write  _restart_ file  (true/false)
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (inout)        :: run_off_tot      !! Diagnosed surface runoff  
                                                                                !! @tex ($kg m^{-2}$) @endtex 
    REAL(r_std),DIMENSION (kjpindex), INTENT (inout)        :: flood_res        !! Flood reservoir estimate 
    REAL(r_std),DIMENSION (kjpindex), INTENT (inout)        :: drainage         !! Diagnosed rainage at the bottom of soil
                                                                                !! @tex ($kg m^{-2}$) @endtex 
...
!_ ================================================================================================================================
  ...
  !! 1.  Initialisation
    ...
    IF (l_first_hydrol) THEN
        ...
        CALL hydrolc_init (kjit, ldrestart_read, kjpindex, index, rest_id, veget, humrel, vegstress, &
             ...
             & zdz1_soil,zdz2_soil,cgrnd_soil,dgrnd_soil,cgrnd_snow,dgrnd_snow,lambda_snow,snowflx,snowcap)
  ...
  !! 2. Preparing restart file for the next simulation
    ...
    END IF ! writing restart
     
  !! 3. Water budget calculations:
    ...
    IF (ok_explicitsnow) THEN 
       ...
       CALL  explicitsnow_main(kjpindex,dtradia,precip_rain,precip_snow,temp_air,pb,u,v,temp_sol_new,soilcap,&
             ...
             & pkappa_snow,lambda_snow,cgrnd_snow,dgrnd_snow,temp_sol_add, veget_max)  !! Arsene 04-03-2014 Add veget_max for snowcompact
    ELSE
    ...
    !! 3.c soil hydrology processes
    CALL hydrolc_soil(kjpindex, vevapnu, precisol, returnflow, reinfiltration, irrigation, tot_melt, mx_eau_var, veget, &
         & ruu_ch, transpir,gqsb, bqsb, dsg, dss, rsol, drysoil_frac, hdry, dsp, runoff, run_off_tot, drainage, humrel, &
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
    ...
    CALL xios_orchidee_send_field("Qs",run_off_tot*0.0005555556)
    CALL xios_orchidee_send_field("Qsb",drainage*0.0005555556)
...
!_ ================================================================================================================================  
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (out)   :: qsintveg            !! Amount of water in the canopy interception 
                                                                                !! reservoir @tex ($kg m^{-2}$) @endtex 
    REAL(r_std),DIMENSION (kjpindex),INTENT(out)         :: grndflux            !! Net flux into the soil
    ...
    REAL(r_std),DIMENSION (kjpindex,nsnow),INTENT(out)   :: snowliq             !! Liquid water content
    REAL(r_std),DIMENSION (kjpindex,nsnow),INTENT(out)   :: snowrho             !! Snow density
    REAL(r_std),DIMENSION (kjpindex,ngrnd),INTENT(out)   :: soiltemp            !! Soil temperature
    REAL(r_std),DIMENSION (kjpindex),INTENT(inout)       :: zdz1_soil
    REAL(r_std),DIMENSION (kjpindex),INTENT(inout)       :: zdz2_soil
    REAL(r_std),DIMENSION (kjpindex),INTENT(inout)       :: cgrnd_soil
    REAL(r_std),DIMENSION (kjpindex),INTENT(inout)       :: dgrnd_soil
...
!_ ================================================================================================================================
  ...
  !! 1. Make sure initialization happens only once (l_first_hydrol becomes false)
  ...
  !! 2. Setting ok_hdiff for horizontal diffusion between soil columns
  ...
  !! 3. Make dynamic allocation with the good dimensions
  ...
  !!  4. Initialization of hydrologic variables:

    !! 4.a Read data from restart input file (opened by sechiba_init)
    ...
    IF (ldrestart_read) THEN
        ...
        IF (ok_explicitsnow) THEN
           ...
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit,.TRUE., snowrho, "gather", nbp_glo, index_g)
           ...
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit,.TRUE.,snowtemp, "gather", nbp_glo, index_g)
           CALL setvar_p (snowtemp, val_exp, 'Snow Temperature profile', tp_00)
           
           var_name = 'soiltemp'
           CALL ioconf_setatt('UNITS', 'K')
           CALL ioconf_setatt('LONG_NAME','Soil Temperature profile')
           CALL restget_p (rest_id, var_name, nbp_glo, ngrnd, 1, kjit,.TRUE.,soiltemp, "gather", nbp_glo, index_g)
           ...
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit, .TRUE.,snowliq, "gather", nbp_glo, index_g)
           ...
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit,.TRUE.,snowheat, "gather", nbp_glo, index_g)
           CALL setvar_p (snowheat, val_exp, 'Snow Heat profile', 0.0)
           
           var_name = 'snowdz'
           CALL ioconf_setatt('UNITS', 'm')
           CALL ioconf_setatt('LONG_NAME','Snow depth profile')
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit,.TRUE.,snowdz, "gather", nbp_glo, index_g)
           CALL setvar_p (snowdz, val_exp, 'Snow depth profile', 0.0)
           
           var_name = 'snowgrain'
           CALL ioconf_setatt('UNITS', 'm')
           CALL ioconf_setatt('LONG_NAME','Snow grain profile')
           CALL restget_p (rest_id, var_name, nbp_glo, nsnow, 1, kjit,.TRUE.,snowgrain, "gather", nbp_glo, index_g)
          ...
          CALL setvar_p (cgrnd_snow, val_exp, 'cgrnd snow coefficient', 0.0)
        ...
        !! 4.b Assign default values if non were found in the restart file
        ...
        !Config Units = [kg/m^2]
        CALL setvar_p (bqsb, val_exp, 'HYDROL_BQSB', wmax_veg*dpu_max)
        ...
        !! 4.c Specific calculations to define default values for dry soil depths : dsp, dss, and hdry
	...
	DO jv = 1,nvm
           zdsp(:,jv) = dpu_max - bqsb(:,jv) / wmax_veg(jv)
        END DO
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
        ...
        !! 4.d Compute vegtot (remember that it is only frac_nobio + SUM(veget(,:)) that is equal to 1)
        DO ji = 1, kjpindex
           vegtot(ji) = SUM(veget(ji,2:nvm)) + tot_bare_soil(ji)
        ENDDO
        
    ENDIF ! IF (ldrestart_read)
    
  !! 5. Where vegetation fraction is zero, set moisture variables to those of bare soil
...
!_ ================================================================================================================================  
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)     :: ruu_ch        !! Volumetric soil water capacity 
                                                                        !! @tex ($kg m^{-3}$) @endtex
    REAL(r_std),DIMENSION (kjpindex,nbdl), INTENT (out):: shumdiag      !! Mean relative soil moisture, diagnosed for 
                                                                        !! thermosoil.f90 (0-1, unitless) 
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)     :: litterhumdiag !! Litter humidity factor (0-1, unitless), used in stomate
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
         IF (jv .EQ. 1) THEN
            mx_eau_var(ji) = mx_eau_var(ji) + tot_bare_soil(ji)*wmax_veg(jv)*dpu_max
         ELSE
            mx_eau_var(ji) = mx_eau_var(ji) + veget(ji,jv)*wmax_veg(jv)*dpu_max
         ENDIF
      END DO
      IF (vegtot(ji) .GT. zero) THEN
      ...
      ENDIF
      ruu_ch(ji) = mx_eau_var(ji) / dpu_max
    END DO
...
!!$       DO jd = 1,nbdl
  ...
  !! 5. Initial value of the fraction of soil which is visibly dry (see hydrolc_soil, 8.3 for details)
  ...
  !! 6. Initial value of the resistance to bare soil evaporation (see hydrolc_soil, 8.4 for details)
    ...
    DO ji = 1, kjpindex
       IF (tot_bare_soil(ji) .GE. min_sechiba) THEN
          ...
          !rsol(ji) =  ( drysoil_frac(ji) + un/(10.*(dpu_max - drysoil_frac(ji))+1.e-10)**2 ) * rsol_cste
          rsol(ji) =  ( hdry(ji) + un/(10.*(dpu_max - hdry(ji))+1.e-10)**2 ) * rsol_cste
       ENDIF
    ENDDO
    
  !! 7. Initial value of litter humidity (see hydrolc_soil, 8.5 for details)
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), INTENT (in)                                 :: dtradia         !! Time step of the simulations for sechiba (s) 
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)            :: precip_rain     !! Rainfall @tex ($kg m^{-2}$) @endtex
...
!!$       ENDDO
    ...
    DO ji=1,kjpindex
      ...
      !! 2.4. Ice melt only if there is more than a given mass : maxmass_snow
   ...
   !! 3. On Land ice
    ...
    DO ji=1,kjpindex
      ...
      !! 3.4 Ice melt only if there is more than a given mass : maxmass_snow, 
      !      i.e. only weight melts glaciers !
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
             ...
             ELSE
                dsg(ji,jv) = (dss(ji,jv) * ruu_ch(ji) + gqsb(ji,jv)) &
                             / ruu_ch(ji)
             ENDIF
             dsp(ji,jv) = dpu_max - bqsb(ji,jv) / ruu_ch(ji)
        ENDIF
      ENDDO
    ENDDO
...
!!
!! MAIN OUTPUT VARIABLE(S) : rsol, drysoil_frac, hdry, runoff (total), 
...
!_ ================================================================================================================================
  ...
  SUBROUTINE hydrolc_soil(kjpindex, vevapnu, precisol, returnflow, reinfiltration, irrigation, tot_melt, mx_eau_var, &
       & veget, ruu_ch, transpir,gqsb, bqsb, dsg, dss, rsol, drysoil_frac, hdry, dsp, runoff, run_off_tot, drainage, &
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), DIMENSION (kjpindex,nvm), INTENT(out)  :: runoff         !! Total runoff @tex ($kg m^{-2}$) @endtex
      ...
      DO ji = 1, kjpindex
        ...
        ELSEIF ((gqsb(ji,jv) + bqsb(ji,jv)).GE.zero) THEN
            ...
            !! dsp is updated accordingly, and defines the depth of top dray soil dss.
                ...
                dsp(ji,jv) = dpu_max - bqsb(ji,jv) / ruu_ch(ji) ! dsp>dpu if bqsb<0 *** we can be here with bsqb<0 and bqsb+gqsb>0
                gqsb(ji,jv) = zero
                dsg(ji,jv) = zero
                dss(ji,jv) = dsp(ji,jv)
            END IF
    ...
    DO jv=1,nvm
      DO ji=1,kjpindex
        ...
        IF ( ABS(dsp(ji,jv)-dsg(ji,jv)) .LT. min_sechiba ) THEN ! min_sechiba = 1.e-8 (constantes.f90)
            bqsb(ji,jv) = bqsb(ji,jv) + gqsb(ji,jv)
            dsp(ji,jv) = dpu_max - bqsb(ji,jv) / ruu_ch(ji) ! *** can this make dsp > dpu_cste
            gqsb(ji,jv) = zero
            dsg(ji,jv) = zero
            dss(ji,jv) = dsp(ji,jv)
        ENDIF
        ...
        dsp(ji,jv) = dpu_max - bqsb(ji,jv)/ruu_ch(ji) ! *** this line shouldn't make dsp>dpu_cste
      ENDDO
    ENDDO
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
              ...
              ELSE IF ( veget(ji,jv) .GT. zero ) THEN
                 !
                 bqsb(ji,jv) = mean_bqsb(ji)
                 dsp(ji,jv) = dpu_max - bqsb(ji,jv)/ruu_ch(ji) ! *** can this line make dsp>dpu_cste ?
              ENDIF
           ENDIF
...
!        ENDDO
        ...
        DO ji = 1, kjpindex
          IF ( lbad(ji,jv) ) THEN
            ...
            !
            gqsb(ji,jv) = zero
            dsp(ji,jv) = dpu_max - bqsb(ji,jv)/ruu_ch(ji)  ! *** could this line make dsp>dpu_cste ? 
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
  ...
  !! 7. Compute total runoff from all soil columns and partition it into drainage and surface runoff
    ...
    DO ji = 1, kjpindex
      IF ( vegtot(ji) .GT. zero ) THEN
!MM veget(:,1) BUG ??!!!!!!!!!!!
       run_off_tot(ji) = run_off_tot(ji) + (runoff(ji,1)*tot_bare_soil(ji))
       DO jv = 2, nvm
      ...
      DO ji=1,kjpindex
         ...
         IF ( gqsb(ji,jv)+bqsb(ji,jv) .GT. zero ) THEN
            ...
            IF (dsg(ji,jv).EQ. zero .OR. gqsb(ji,jv).EQ.zero) THEN
               humrel(ji,jv) = EXP( - humcste(jv) * dpu_max * (dsp(ji,jv)/dpu_max) )
               dsg(ji,jv) = zero
               ...
               ! *** is around dpu
               IF (dsp(ji,jv).GT.(dpu_max - min_sechiba) .OR. bqsb(ji,jv).LT.zero) THEN
                  humrel(ji,jv) = zero
               ENDIF
...
!_ ================================================================================================================================   
  ...
  !! 0. Variable and parameter declaration
    ...
    LOGICAL, INTENT (in)                                   :: first_call     !! This flag is set to true to initialize the total 
                                                                             !! amount of water at beginning of time step 
                                                                             !! (true/false)
    REAL(r_std), INTENT (in)                               :: dtradia        !! Time step duration (s)
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)      :: veget          !! Grid-cell fraction effectively covered by 
                                                                             !! vegetation for each PFT, except for soil 
...
!_ ================================================================================================================================
  ...
  !! 2. We check the water balance : this is done at the end of a time step
    
    tot_water_end(:) = zero
    
    !! 2.1 If the "nobio" fraction does not complement the "bio" fraction, we issue a warning
    ...
    !! 2.2 We calculate the total amount of water in grid-cells at the end the time step
    watveg(:) = zero
    sum_vevapwet(:) = zero
    sum_transpir(:) = zero
    sum_snow_nobio(:) = zero
    ...
    !!  over the time step dtradia, with a unit of kg m^{-2}.
    DO ji = 1, kjpindex
    ...
    ENDDO
       ...
       !! 2.4 We do the check, given some minimum required precision
  ...
  !! 3. Transfer the total water amount at the end of the current timestep to the begining of the next one
...
!_ ================================================================================================================================   
  ...
  !! 0. Variable and parameter declaration
    ...
    LOGICAL, INTENT (in)                                :: first_call  !! This flag is set to true to initialize the water amount 
                                                                       !! variables at beginning of time step (true/false)
...
!_ ================================================================================================================================
  ...
  !! 2. We calculate the required variables at the end of each time step 
  ...
  !! 4. We calculate the mean relative humidity of soil in the grid-cells
...
!$OMP THREADPRIVATE(tau_hdiff)
    LOGICAL, SAVE                                        :: firstcall=.TRUE. !! Flag to set tau_hdiff at the first time step 
  ...
  !! 3.  Relax PFT values towards grid-cell mean
    ...
    DO jv = 1, nvm
      DO ji = 1, kjpindex
        ...
        ENDIF
        dsp(ji,jv) = dpu_max - bqsb(ji,jv) / ruu_ch(ji) ! no change here since bqsb does not change
