...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_sechiba/routing.f90 $
!! $Date: 2014-06-12 12:15:32 +0200 (Thu, 12 Jun 2014) $
!! $Revision: 2183 $
!! \n
!_ ================================================================================================================================
...
MODULE routing
  ...
  !
  LOGICAL, SAVE                                              :: l_first_routing=.TRUE.      !! Logical to initialize the routine. Initialisation has to be done one time (true/false)
...
!$OMP THREADPRIVATE(diagunit)
  ...
  !
  LOGICAL, SAVE                                              :: doirrigation = .FALSE.      !! Logical to choose if irrigation is activated or not (true/false)
!$OMP THREADPRIVATE(doirrigation)
  LOGICAL, SAVE                                              :: dofloodplains = .FALSE.     !! Logical to choose if floodplains are activated or not (true/false)
!$OMP THREADPRIVATE(dofloodplains)
...
!! \endlatexonly
!! \n
! <JR19012> Could there be more labels on the picture to make it clear what does V_1, V_2 etc correspond to?
!_ ================================================================================================================================

SUBROUTINE routing_main(kjit, nbpt, dtradia, control_flags, ldrestart_read, ldrestart_write, index, &
...
!! INPUT VARIABLES
    ...
    REAL(r_std), INTENT(in)        :: dtradia              !! Time step of the model (s)
    TYPE(control_type), INTENT(in) :: control_flags        !! Flags that (de)activate parts of the model (unitless)
    LOGICAL, INTENT(in)            :: ldrestart_read       !! Logical for _restart_ file to read (true/false)
    LOGICAL, INTENT(in)            :: ldrestart_write      !! Logical for _restart_ file to write (true/false)
...
!! LOCAL VARIABLES
    ...
    LOGICAL, SAVE                  :: init_irrig=.FALSE.   !! Logical to initialize the irrigation (true/false)
!$OMP THREADPRIVATE(init_irrig)
    LOGICAL, SAVE                  :: init_flood=.FALSE.   !! Logical to initialize the floodplains (true/false)
!$OMP THREADPRIVATE(init_flood)
    LOGICAL, SAVE                  :: init_swamp=.FALSE.   !! Logical to initialize the swamps (true/false)
!$OMP THREADPRIVATE(init_swamp)
...
!_ ================================================================================================================================
    ...
    IF (l_first_routing) THEN
       ...
       !
       CALL routing_init (kjit, nbpt, index, control_flags, dtradia, returnflow, reinfiltration, irrigation, &
    ...
    !
    ! Averaged variables (i.e. *dtradia/dt_routing). This accounts for the difference between the shorter
    ! timestep dtradia of other parts of the model and the long dt_routing timestep (set to one day at present)
    !
    ...
    IF ( NINT(time_counter) .GE. NINT(dt_routing) ) THEN 
       ...
       !
       ! Change units as above but at the same time transform the kg/dtradia to m^3/dtradia
       !
       riverflow_mean(:) = riverflow_mean(:)/dt_routing*dtradia/mille
       coastalflow_mean(:) = coastalflow_mean(:)/dt_routing*dtradia/mille
       hydrographs(:) = hydrographs(:)/dt_routing*dtradia/mille
       !
       !
    ENDIF
    !
    ...
    IF (ldrestart_write) THEN 
       ...
       var_name = 'floodh'
       CALL restput_p (rest_id, var_name, nbp_glo, 1, 1, kjit, flood_height, 'scatter',  nbp_glo, index_g)
       ...
       CALL restput_p (rest_id, var_name, nbp_glo, 1, 1, kjit, transpot_mean, 'scatter',  nbp_glo, index_g)
       ...
       CALL restput_p (rest_id, var_name, nbp_glo, 1, 1, kjit, k_litt_mean, 'scatter',  nbp_glo, index_g)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE routing_init(kjit, nbpt, index, control_flags, dtradia, returnflow, reinfiltration, irrigation, &
       &                  riverflow, coastalflow, flood_frac, flood_res, stempdiag, rest_id)
    !
    IMPLICIT NONE
    !
...
!! LOCAL VARIABLES
    ...
    REAL(r_std), DIMENSION(1)                    :: tmp_day        !!
    REAL(r_std)                                  :: ratio          !! Diagnostic ratio to check that dt_routing is a multiple of dtradia (unitless)
    REAL(r_std)                                  :: totarea        !! Total area of basin (m^2)
    INTEGER(i_std)                               :: ier, ig, ib, ipn(1) !! Indices (unitless)
    LOGICAL                                      :: hydrol_cwrr    !! Flag to select the CWRR hydrology (true/false)

!_ ================================================================================================================================
    !
    ! Taken from overall control of model
    !
    hydrol_cwrr = control_flags%hydrol_cwrr
    doirrigation = control_flags%do_irrigation
    dofloodplains = control_flags%do_floodplains
    !
    !
...
!!$    CALL setvar_p (time_counter, val_exp, 'NO_KEYWORD', zero)
    ...
    ! nbintobasin
    !
    ALLOCATE (route_nbintobas_loc(nbpt,nbasmax))
    ALLOCATE (route_nbintobas_glo(nbp_glo,nbasmax))
    IF (is_root_prc) THEN
...
!    flood_res = zero
    ...
    IF (ier.NE.0) THEN
        WRITE (numout,*) ' error in hydroupbasin allocation. We stop. We need kjpindex words = ', nbpt*nbasmax
        STOP 'routing_init'
    END IF
...
!! LOCAL VARIABLES
    ...
    INTEGER(i_std)                               :: ig, ib, ib2, ig2, rtg, rtb, in !! Indices (unitless)
    !
    ...
    !
    LOGICAL                                      :: error_reservoir           !! Logical to check if there is a negative amount of water in the slow reservoir (true/false)
    !
    !! PARAMETERS
...
!_ ================================================================================================================================
    ...
    ALLOCATE(totarea(num_largest), tmparea(num_largest))
    ALLOCATE(topids(num_largest))
    ALLOCATE(sortedrivs(num_largest))
    ALLOCATE(sorted_names(num_largest))
    ALLOCATE(streams_nb(num_largest), streams_avehops(num_largest), streams_minhops(num_largest))
    ALLOCATE(streams_maxhops(num_largest))
    ALLOCATE(streams_resid(num_largest))
    !
    ALLOCATE(lbasin_area(num_largest,nbpt), lbasin_uparea(num_largest,nbpt), lrivercode(num_largest,nbpt))
    !
    ...
    !Config Units = [FILE]
    !
    river_file_name="river_desc.txt"
    ...
    !
    nbmax=MAXVAL(nb_pts)
    ALLOCATE(allstreams(nbmax, longest_river), upstreamchange(nbmax, longest_river))
    ALLOCATE(tstreams(longest_river), tupstreamchange(longest_river))
    ALLOCATE(tslen(nbmax), tpts(nbmax), tptbas(nbmax), tuparea(nbmax))
    ALLOCATE(tcode(nbmax))
    !
    DO icc = 1, num_largest
       !
...
!_ ================================================================================================================================
    ...
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of lat_rel (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE(lon_rel(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of lon_rel (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    !
    ALLOC_ERR=-1
    ...
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of trip (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (basins(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of basins (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (topoindex(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of topoindex (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (hierarchy(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of hierarchy (routing_basins): ",ALLOC_ERR
      STOP 
    ENDIF
    !
    !
    ...
    IF (ALLOC_ERR/=0) THEN
       WRITE(numout,*) "ERROR IN ALLOCATION of sub_pts : ",ALLOC_ERR
       STOP 
    ENDIF
    sub_pts(:)=0
    !
    ...
    !
    ALLOCATE (area_bx(nbvmax,nbvmax))
    ALLOCATE (hierarchy_bx(nbvmax,nbvmax))
    ALLOCATE (lon_bx(nbvmax,nbvmax))
    ALLOCATE (lat_bx(nbvmax,nbvmax))
    ALLOCATE (topoind_bx(nbvmax,nbvmax))
    ALLOCATE (trip_bx(nbvmax,nbvmax))
    ALLOCATE (basin_bx(nbvmax,nbvmax))
    ALLOCATE (coast_pts(nbvmax))
    !
    ALLOCATE (basin_inbxid(nbvmax))
    ALLOCATE (basin_sz(nbvmax))
    ALLOCATE (basin_pts(nbvmax,nbvmax,2))
    ALLOCATE (basin_bxout(nbvmax))
    !
...
SUBROUTINE routing_irrigmap (nbpt, index, lalo, neighbours, resolution, contfrac, &
    ...
    !
!! PARAMETERS
    INTEGER(r_std), PARAMETER                      :: ilake = 1             !! Number of type of lakes area (unitless)
    INTEGER(r_std), PARAMETER                      :: idam = 2              !! Number of type of dams area (unitless)
    INTEGER(r_std), PARAMETER                      :: iflood = 3            !! Number of type of floodplains area (unitless)
    INTEGER(r_std), PARAMETER                      :: iswamp = 4            !! Number of type of swamps area (unitless)
    INTEGER(r_std), PARAMETER                      :: isal = 5              !! Number of type of salines area (unitless)
    INTEGER(r_std), PARAMETER                      :: ipond = 6             !! Number of type of ponds area (unitless)
    INTEGER(r_std), PARAMETER                      :: ntype = 6             !! Number of types of flooded surfaces (unitless)
...
!_ ================================================================================================================================
    ...
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of latrel : ",ALLOC_ERR, iml, jml
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (lonrel(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of lonrel : ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (irrigated_frac(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of irrigated_frac : ",ALLOC_ERR
      STOP 
    ENDIF
    ALLOC_ERR=-1
    ALLOCATE (flood_fracmax(iml,jml,ntype), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
      WRITE(numout,*) "ERROR IN ALLOCATION of flood_fracmax : ",ALLOC_ERR
      STOP 
    ENDIF
    !
...
!          ENDIF
    ...
    !
    ALLOC_ERR=-1
    ALLOCATE(sub_index(nbpt, nbpmax, 2), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
       WRITE(numout,*) "ERROR IN ALLOCATION of sub_index : ",ALLOC_ERR
       STOP 
    ENDIF
    sub_index(:,:,:)=0
    ALLOC_ERR=-1
    ALLOCATE(sub_area(nbpt, nbpmax), STAT=ALLOC_ERR)
    IF (ALLOC_ERR/=0) THEN
       WRITE(numout,*) "ERROR IN ALLOCATION of sub_area : ",ALLOC_ERR
       STOP 
    ENDIF
    ...
    CALL aggregate_p(nbpt, lalo, neighbours, resolution, contfrac, &
         ...
         &                nbpmax, sub_index, sub_area, ok_interpol)
    !
    !
    WHERE (sub_area < 0) sub_area=zero
    !  
    ...
    DO ib=1,nbpt
       ...
       fopt=1
       !
       DO WHILE ( sub_area(ib,fopt) > zero ) 
          !
          ip = sub_index(ib, fopt, 1)
          jp = sub_index(ib, fopt, 2)
          !
          IF (irrigated_frac(ip,jp) .LT. undef_sechiba-1.) THEN
             area_irrig = area_irrig + sub_area(ib,fopt)*irrigated_frac(ip,jp)
          ENDIF
          !
          DO itype=1,ntype
             IF (flood_fracmax(ip,jp,itype) .LT. undef_sechiba-1.) THEN
                area_flood(itype) = area_flood(itype) + sub_area(ib,fopt)*flood_fracmax(ip,jp,itype)
             ENDIF
          ENDDO
          !
          fopt = fopt + 1
...
!! INPUT VARIABLES
    INTEGER(i_std), INTENT(in) :: nbpt                 !! Domain size  (unitless)
    LOGICAL, INTENT(in)        :: firstcall            !! Controls behaviour (true/false)
    REAL(r_std), INTENT(in)    :: floodout(nbpt)       !! Grid-point flow out of floodplains (kg/m^2/dt)
    REAL(r_std), INTENT(in)    :: runoff(nbpt)         !! Grid-point runoff (kg/m^2/dt)
    REAL(r_std), INTENT(in)    :: drainage(nbpt)       !! Grid-point drainage (kg/m^2/dt)
