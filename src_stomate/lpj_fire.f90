...
!!
!! SVN           :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_stomate/lpj_fire.f90 $ 
...
!! burned. From this part of residue, there is further certain fraction that 
!! goes to black carbon pool (very inert carbon pool that is part of geological
!! carbon cycle), and the remaining part is transferred to (in case of biomass) or
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declarations
    ...
    REAL(r_std), DIMENSION(npts)                                  :: bcfrac             !! Fraction of residue transformed into 
                                                                                        !! black carbon (unitless, 0-1) 
...
!_ ================================================================================================================================
 ...
 !! 2. Determine fire probability
  ...
  !! 4. Determine fire impact
       ...
       !      The burned part goes to either the atmposhere, or litter, or blackcarbon pool. 
       DO k = 1, nparts
          ...
          IF ( .NOT. ( ( .NOT. is_tree(j)  .OR. .NOT. is_shrub(j) ) .AND. ( ( k.EQ.iroot ) .OR. ( k.EQ.icarbres ) ) ) ) THEN  !! Arsene 31-07-2014 modifications ok
             ...
             !! 4.2.2.1 Determine fraction of black carbon 
             !          Only for plant parts above the ground, i.e. when co2_frac > 0. A small part of 
             !          the residue, which can be expressed as a function of the fraction of volatilized 
             !          carbon (assimilated to co2frac here), becomes black carbon, and thus withdrawn 
             !          from the soil carbon cycle (added to the "geologic carbon cycle we don't care 
             !          about here).
             !             IF ( co2frac(k) .GT. min_stomate ) THEN
             IF ( co2frac(k) .GT. zero ) THEN
                bcfrac(:) = bcfrac_coeff(1) / ( bcfrac_coeff(2) ** ( bcfrac_coeff(3) - 100.*co2frac(k) ) + un )
             ELSE
                bcfrac(:) = zero
             ENDIF

             !! 4.2.2.2 Add fraction of the residue to the black carbon "reservoir"
             black_carbon(:) = &
                  black_carbon(:) + bcfrac(:) * residue(:)* veget_max(:,j)

             !! 4.2.2.3 The rest (largest part) of the residue becomes litter.
             bm_to_litter(:,j,k,icarbon) = bm_to_litter(:,j,k,icarbon) + residue(:) * ( 1 - bcfrac(:) )
  ...
  !! 5. Burn litter
    ...
    !   with its lignin content. The burned part either goes to the atmosphere, or to the 
    !   black carbon pool, or remains in litter as unburned residue.
    DO j = 2,nvm  !loop over #PFT
       ...
       !      residues go to either black carbon pool, or remain as litter.
       ...
       bcfrac(:) = bcfrac_coeff(1)/ ( bcfrac_coeff(2) ** ( bcfrac_coeff(3) - 100.*(un - struc_residual(:)) ) + un )

       !! 5.2.5 Add this fraction of the residue to the black carbon "reservoir"
       !        In gC m^{-2}
       black_carbon(:) = &
            black_carbon(:) + bcfrac(:) * residue(:)* veget_max(:,j)
