...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_stomate/lpj_light.f90 $
...
!!
!! The flow in the routine is different for ::control%ok_dgvm. When ::control%ok_dgvm is true
...
!!
!! When ::control%ok_dgvm is false then light competition is calculated for the static case if the mortality is not 
...
!!!$                ENDIF
       ...
       DO i = 1, npts ! SZ why this loop and not a vector statement ?
          ...
          IF ( sumfpc(i) .GT. fpc_crit ) THEN
             ...
             !SZ             IF (sumfpc_wood .GE. fpc_crit ) THEN
             !
