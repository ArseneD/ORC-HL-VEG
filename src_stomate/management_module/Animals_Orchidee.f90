...
!  USE parallel
  ...
  REAL(r_std), PARAMETER :: fnurine        = 0.6  ! repartition de n dans l'urine et les fèces (-)
  REAL(r_std), PARAMETER :: kintake        = 1.0  ! parameter zu intake (m**2/m**2)
  REAL(r_std), PARAMETER :: fmethane       = 0.03 ! c-pertes en méthane (-)
  REAL(r_std), PARAMETER :: AnimalqintakeM = 3.0
  REAL(r_std), PARAMETER :: franimal       = 0.5  ! c-pertes en respiration (-)
  ...
  REAL(r_std ), PARAMETER :: t_seuil_OMD = 288.15  ! threshold temperature for calculation of temperature effect on OMD (K)        
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!! Variables locales au module
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milk
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkn         ! n dans le lait (kg n /(m**2*d))
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkc         ! c dans le lait (kg c /(m**2*d))     
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ranimal       ! c perte en respiration (kg c /(m**2*d))
  ...
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementc ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementn ! N flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementcsum ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementnsum ! N flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  ...
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: methane_ani        ! Enteric methane emission per animal(kg C animal-1 d-1)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: methane_aniSum     ! Annual enteric methane emission per animal(kg C animal-1 )
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkanimalSum      ! Annual milk production per animal(kg C animal-1 )
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkanimal
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ugb                ! equals 0 (no animals) or 1 (animals)
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ok_ugb             ! 1 if autogestion is optimal; 0 else
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: extra_feed     ! Forage necessary to feed animals at barn when stocking rate autogestion (kg DM m-2)

  !local module Variables for cow (npts,2) for young and adult cow
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  Wanimalcow       ! Animal liveweight (kg/animal) (young:1, adult:2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  BCScow           ! Body score condition cow (young in first, and adult in second) (/5)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  BCScow_prev      ! previous Body score condition cow (young in first, and adult in second) (/5)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  AGEcow           ! Age of cow (necessary for dairy cow and not necessary for suckler cow) (month)

  !Local modul variable for complementation
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Forage_quantity_period ! forage quantity for the current grazing period (Kg/Animal/d)

  !local module variable for milk productivity cow
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowsum        ! Annual milk production of cows (young in first, and adult in second)(kg/y) 
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcow2sum       ! Annual milk production of a cow (young in first, and adult in second)(kg/animal/d)     
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcow2_prec     ! Daily actual milk production per animal for primiparous or multiparous cows at previous time step (kg/animal/d)


  !local modul variable for Bilan N C cow 
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowN           ! N in daily milk production per m2 for primiparous or multiparous cows (kgN/m2/d)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowC           ! C in daily milk production per m2 for primiparous or multiparous cows (kgC/m2/d)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowCsum        ! Cumulated C in milk production per m2 for primiparous or multiparous cows (kgC/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowNsum        ! Cumulated N in milk production per m2 for primiparous or multiparous cows (kgN/m2)

  !Intake cow
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowsum        ! Cumulated intake per m2 for primiparous or multiparous cows(kg/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowNsum       ! N in Cumulated intake per m2 for primiparous or multiparous cows(kgN/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowCsum       ! C in Cumulated intake per m2 for primiparous or multiparous cows(kgC/m2)    
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowanimalsum  ! Cumulated animal intake for primiparous or multiparous cows(kg/animal)
  !local module variable for calves
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wanimalcalf            ! Calf liveweigth (kg/animal)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfsum             ! Cumulated calf intake per m2(kg/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfnsum            ! N in cumulated calf intake per m2(kgN/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfanimalsum       ! Cumulated calf intake per animal kg/animal)  

  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Tcalving               ! Calving date (d)  
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Tsevrage               ! Suckling period of calves (d)  
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Age_sortie_calf        ! Calf age at sale (d)  
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Pyoung                 ! Fraction of young or primiparous in the cattle (-)  
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wcalfborn              ! Calf liveweigth at birth (kg/animal)
  INTEGER,      ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  calfinit               ! Boolean to calf weight computation
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wanimalcalfinit        ! Initial calf liveweigth (kg/animal) (birth liveweight or liveweight at the beginning of the grazing period)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  nanimaltot_prec        ! nanimaltot at previous time step (animal/m2)

  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Gestation               ! equals 0 (outside of the gestation period) or 1 (during gestation)
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Calf                    ! equals 0 (when calves are sale or at barn) or 1 (when calves are at pasture)

  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: compte_pature           ! Number of the pasture periode when stocking rate automanagement
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_weightcow   ! Initial cow liveweight when stocking rate automanagement (kg/animal)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_BCScow      ! Initial BCS when stocking rate automanagement (-)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_AGEcow      ! Initial age when stocking rate automanagement (months)
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: autogestion_init        ! to intialize cow liveweight and BCS the first time step when f_autogestion=2
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: QIc                     ! to intialize concentrate amount per kg of milk per day or per kg of Liveweight per day (Kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: EVf                     ! to intialize forage energy content  (UF/kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: FVf                     ! to intialize forage fill value  (UE/kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: EVc                     ! to intialize concentrate energy content(UF/kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: fN_forage               ! Nitrogen fraction in the forage (kgN/kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:)   :: fN_concentrate          ! Nitrogen fraction in the concentrate (kgN/kg)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: NEBcow_prec             ! Net energy Balance at previous time step (young:1, mature:2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: MPwmax                  ! Maximum of theoretical milk production (kg/animal/d)
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Fday_pasture            ! the first julian day of the actual pasture periode
  INTEGER(i_std)     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: delai_ugb               ! time before start grazing is possible
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: Local_autogestion_out   ! Fraction F (npts,1), ratio F (npts,2), and lenght of the grazing period when autgestion
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: PEmax                   ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: PEpos                   ! Perte d'etat possible des vaches laitières au jour j
  REAL(r_std),              SAVE                 :: BM_threshold            ! Biomass threshold above which animals are moved out the paddock (kg/m2)
  REAL(r_std),              SAVE                 :: BM_threshold_turnout    ! [autogestion] Biomass threshold above which animals are moved in the paddock (kg/m2)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: DMIc                    ! concentrate ingested with auto-complementation (dairy cow only)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: DMIf                    ! forage ingested with auto-complementation (suckler cow only)

  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: NER                     ! Net energy requirement (MJ)
...
!!!!!!!!!!!!!!!!
    ...
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: devstage    ! stade de développement
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: wsh         ! totalité de masse sèche structurelle des pousses  (kg/m**2)  ----> total structural dry mass of shoots
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: intakemax   ! Potential eating rate of lactating cows (kg/(GVE*m**2)       ----> potential intake
    REAL(r_std), DIMENSION(npts)          , INTENT(in)    :: snow        ! neige                                                        ----> snow  
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: wshtot      ! totalité de masse sèche  de la pousse (kg/m**2)              ----> total dry mass of the shoots
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: Animalwgrazingmin  !  ????----> LiLH
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: AnimalkintakeM     !  ????----> LiLH
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: nel         ! énergie nette de lactation (mj/kg)
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: wanimal     ! weight of lactating cows (kg)
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(inout) :: nanimaltot   ! densité de paturage (gve/m**2)
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: ntot        ! concentration totale en n 
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: intake      ! intake
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: urinen      ! n dans l'urine (kg n /(m**2 d))     
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: faecesn     ! n dans les fèces (kg n /(m**2*d)) 
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: urinec      ! c dans les urines
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: faecesc     ! c dans les fèces (kg c /(m**2*d))
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(in)    :: tgrowth     ! instant de la repousse
    LOGICAL                              , INTENT(in)    :: new_year
    LOGICAL                              , INTENT(in)    :: new_day                            
    INTEGER(i_std)                              , INTENT(in)    :: tcutmodel   ! flag for management 
    INTEGER(i_std)                           , INTENT(in)    :: tjulian     ! day julian
    REAL(r_std), DIMENSION(npts,nvm,nstocking), INTENT(in)    :: nanimal  ! densité du paturage  h (1,..,nstocking) (gve/m**2)
    REAL(r_std), DIMENSION(npts,nvm,nstocking), INTENT(in)    :: tanimal  ! début du paturage    h (1,..,nstocking) (d)       
    REAL(r_std), DIMENSION(npts,nvm,nstocking), INTENT(in)    :: danimal  ! durée du paturage    h (1,..,nstocking) (d)       
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)    :: import_yield        ! rendement de la prairie fauchee (g m-2 yr-1) (autogestion NV runs saturant nonlimitant)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)  :: intakesum          ! Yearly intake (kg animal-1 y-1)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)  :: intakensum         ! N in daily intake per m2(kgN/m2)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: fn                 ! nitrogen in structural dry matter
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: n                  ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: c
    REAL(r_std), DIMENSION(npts,nvm,nleafages), INTENT(inout):: leaf_frac
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)    :: intake_animal      ! Daily intake per animal(kg animal-1 d-1)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)  :: intake_animalsum   ! Yearly intake per animal(kg animal-1 d-1)
    REAL(r_std), DIMENSION(npts,nvm,nparts,nelements), INTENT(inout):: biomass       ! totalité de masse sèche du shoot(kg/m**2)
...
!JCADD for autogestion 5 grazing AGB and litter
    ...
    INTEGER(i_std), DIMENSION(npts,nvm), INTENT(out)  ::  grazing_litter
...
! local Variables:
    ...
    REAL(r_std)     , DIMENSION(npts,nvm)   :: nb_ani_old     ! Actual stocking rate per ha of total pasture "D" at previous iteration (animal (ha of total grassland)-1)
              ...
              DO j=2,nvm
                IF (is_grassland_manag(j) .AND. (.NOT.  is_c4(j)) .AND.  &
                  ...
                  WHERE ((ok_ugb(:,j) .EQ. 0))
                    ...
                    extra_feed(:,j)  = (365 - compt_ugb(:,j)) * 13 * sr_ugb(:,j) 
        ...
        IF(f_nonlimitant .EQ. 0) THEN
            ...
            IF (f_autogestion .EQ. 3) THEN
                WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0))
                    extra_feed(:,mgraze_C3)  = (365 - compt_ugb(:,mgraze_C3)) * 13 *sr_ugb(:,mgraze_C3)
                ...
                WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0))
                    extra_feed(:,mgraze_C4)  = (365 - compt_ugb(:,mgraze_C4)) * 13 *sr_ugb(:,mgraze_C4)
...
print *, 'frac',grazed_frac(:,mgraze_C3)
                ...
                WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0) .AND. (sr_ugb(:,mgraze_C3) .GT. 0.0))
                  ...
                  extra_feed(:,mgraze_C3)  = (365 - compt_ugb(:,mgraze_C3)) * 13.0*sr_ugb(:,mgraze_C3)
                  ...
                  consump(:,mgraze_C3) = (365 - compt_ugb(:,mgraze_C3)) * 13.0 * nb_ani(:,mgraze_C3)
                  ...
                  add_nb_ani(:,mgraze_C3) = outside_food(:,mgraze_C3)/ (13.0 * 365) * 0.2
...
print *, 'frac_post',grazed_frac(:,mgraze_C3)
                WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0) .AND. (sr_ugb(:,mgraze_C4).GT. 0.0))
                  ...
                  extra_feed(:,mgraze_C4)  = (365 - compt_ugb(:,mgraze_C4)) *13.0*sr_ugb(:,mgraze_C4)
                  ! total yield of las year (kg DM/m^2 total grassland)
                  amount_yield(:,mgraze_C4) = import_yield(:,mgraze_C4) *(1-grazed_frac(:,mgraze_C4)) * 0.85
                  ! total animal indoor consumption of last year (kg DM/m^2 total grassland)       
                  consump(:,mgraze_C4) = (365 - compt_ugb(:,mgraze_C4)) * 13.0 *nb_ani(:,mgraze_C4)
                  ! food surplus (outside_food > 0) or deficit (outside_food < 0)
                  outside_food(:,mgraze_C4) = amount_yield(:,mgraze_C4)-consump(:,mgraze_C4)
                  ! farmers' decision of buy (add_nb_ani > 0) or sell (add_nb_ani < 0) animals 
                  add_nb_ani(:,mgraze_C4) = outside_food(:,mgraze_C4)/ (13.0 *365) * 0.2
                  !! New animal density for total grassland
            ...
            IF (f_postauto .EQ. 5) THEN
                WHERE ((ok_ugb(:,mgraze_C3) .EQ. 0) .AND. &
                  ...
                  amount_yield(:,mgraze_C3) = import_yield(:,mgraze_C3) * (1-grazed_frac(:,mgraze_C3)) * 0.85
                  ! total animal indoor consumption of last year (kg DM/m^2 total grassland)
                  consump(:,mgraze_C3) = 0.0 !(365 - compt_ugb(:,mgraze_C3)) * 13.0 * nb_ani(:,mgraze_C3)
                  ! food surplus (outside_food > 0) or deficit (outside_food < 0)
                  outside_food(:,mgraze_C3) = 0.0 !amount_yield(:,mgraze_C3)-consump(:,mgraze_C3)
                  ! farmers' decision of buy (add_nb_ani > 0) or sell (add_nb_ani < 0) animals
                  add_nb_ani(:,mgraze_C3) = 0.0 !outside_food(:,mgraze_C3)/ (13.0 * 365) * 0.2
                  !! New animal density for total grassland
                  nb_ani(:,mgraze_C3)=nb_ani(:,mgraze_C3)+add_nb_ani(:,mgraze_C3)
                  !! New fraction of grazed grassland in total grassland (keep the same stocking rate)
                  WHERE (sr_ugb(:,mgraze_C3) .GT. 0.0)
...
!                  ENDWHERE
                ...
                WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0) .AND. (sr_ugb(:,mgraze_C4).GT. 0.0))
                  ...
                  amount_yield(:,mgraze_C4) = import_yield(:,mgraze_C4) *(1-grazed_frac(:,mgraze_C4)) * 0.85
                  ! total animal indoor consumption of last year (kg DM/m^2 total grassland)
                  consump(:,mgraze_C4) = 0.0 !(365 - compt_ugb(:,mgraze_C4)) * 13.0 *nb_ani(:,mgraze_C4)
                  ! food surplus (outside_food > 0) or deficit (outside_food < 0)
                  outside_food(:,mgraze_C4) = 0.0 !amount_yield(:,mgraze_C4)-consump(:,mgraze_C4)
                  ! farmers' decision of buy (add_nb_ani > 0) or sell (add_nb_ani < 0) animals
                  add_nb_ani(:,mgraze_C4) = 0.0 !outside_food(:,mgraze_C4)/ (13.0 *365) * 0.2
                  !! New animal density for total grassland
                  nb_ani(:,mgraze_C4)=nb_ani(:,mgraze_C4)+add_nb_ani(:,mgraze_C4)
                  !! New fraction of grazed grassland in total grassland (keep
                  !the same stocking rate)
...
!                             .AND. (f_postauto .EQ. 0)) THEN
        calc_nanimaltot  : IF ((tcutmodel .EQ. 0) .AND.  (f_autogestion .EQ. 0) &
            ...
            DO WHILE(h .LT. nstocking)
              WHERE((tjulian .GE. tanimal(:,:,h)) .AND. (tjulian .LT. (tanimal(:,:,h) + danimal(:,:,h))))
            ...
            WHERE (wshtot(:,:) .GE. (min_grazing + 0.05))
              ...
              ELSEWHERE
                ugb(:,:) = 0
              ENDWHERE
            ELSEWHERE ((wshtot(:,:) .LT. (min_grazing + 0.05)) .AND. (wshtot(:,:) .GE. min_grazing))
              WHERE ((delai_ugb(:,:) .GE. 0) .AND. (nanimaltot(:,:) .GT. 0.0))
                ugb(:,:) = 1
              ELSEWHERE
                ugb(:,:) = 0
        ...
        ELSEIF (tcutmodel .EQ. 1) THEN
            ...
            WHERE ((nanimal(:,:,1) .GT. 0.0) .AND. (devstage(:,:) .GT. devstocking) .AND. (stockingstart(:,:) .EQ. 0))
          ...
          DO j=2,nvm
            IF (tjulian .GT. tseasonendmin) THEN
                WHERE ((stockingstart(:,j) .EQ. 1) .AND. (stockingend(:,j) .EQ. 0) .AND. (snow(:) .GT. 0.0))
          ...
          END DO
            ...
            ELSEWHERE ( (nanimal(:,:,1) .GT. 0.0) .AND. (stockingstart(:,:) .EQ. 1))
        ...
        IF (f_autogestion .EQ. 2) THEN
          DO j=2,nvm
            IF (is_grassland_manag(j) .AND. (.NOT. is_grassland_cut(j)).AND.(.NOT.is_grassland_grazed(j)))THEN
...
!print *,'able_grazing', able_grazing(301:320,mgraze_C3)
          ...
          ELSEWHERE (sr_ugb(:,mgraze_C3) .LT. 0.00002)
            ...
            END WHERE
          ENDWHERE
!print *,'wshtot', wshtot(301:320,mgraze_C3)
!print *,'nn1', nanimaltot(301:320,mgraze_C3)
!print *,'sr_ugb', sr_ugb(301:320,mgraze_C3)
!print *,'delai',delai_ugb(301:320,mgraze_C3)
!print *,'ugbbefore',ugb(301:320,mgraze_C3)
            IF (tjulian .GT. tseasonendmin) THEN
              WHERE (snow(:) .GT. 0.0)
                nanimaltot (:,mgraze_C3) = 0.0
                ugb(:,mgraze_C3)           = 0
            ...
            WHERE (ugb(:,mgraze_C3) .EQ. 1)
              ...
              ENDWHERE
            END WHERE
!if (nanimaltot(304,mgraze_C3) .GT. 0.0) then
!print*,'grazingstart'
!endif
!print *,'snow',snow(301:320)
!print *,'ugbafter',ugb(301:320,mgraze_C3)
          ! > 1 LSU/ha using 0.25 kgDM
...
! 0.01 LSU 5 gDM  
        IF (f_autogestion .EQ. 5) THEN
          ...
          WHERE (sr_ugb(:,mgraze_C3) .GE. 0.0001)
            ...
            ELSEWHERE (wshtot(:,mgraze_C3) .LT. (min_grazing - 0.075) .AND. &
                      & litter_avail_totDM(:,mgraze_C3) .GE. (min_grazing + 0.05))
                ! can grazing
                ugb(:,mgraze_C3) = 1
                ! grazing litter
                grazing_litter(:,mgraze_C3) = 1
            ELSEWHERE (wshtot(:,mgraze_C3) .LT. (min_grazing - 0.075) .AND. &
                 & litter_avail_totDM(:,mgraze_C3) .LT. (min_grazing - 0.075))
                ! cannot grazing
                ugb(:,mgraze_C3) = 0
                ! no grazing
          ...
          ELSEWHERE (sr_ugb(:,mgraze_C3) .GE. 0.00002 .and. sr_ugb(:,mgraze_C3) .LT. 0.0001)
            WHERE (wshtot(:,mgraze_C3) .GE. able_grazing(:,mgraze_C3))
                ...
                grazing_litter(:,mgraze_C3) = 0
            ELSEWHERE (wshtot(:,mgraze_C3) .LT. able_grazing(:,mgraze_C3)*0.5 &
                 & .AND. litter_avail_totDM(:,mgraze_C3) .GE. able_grazing(:,mgraze_C3))
                ! can grazing
                ugb(:,mgraze_C3) = 1
                ! grazing litter
                grazing_litter(:,mgraze_C3) = 1
            ELSEWHERE (wshtot(:,mgraze_C3) .LT. able_grazing(:,mgraze_C3)*0.5 &
                 & .AND. litter_avail_totDM(:,mgraze_C3) .LT. able_grazing(:,mgraze_C3)*0.5)
                ! cannot grazing
                ugb(:,mgraze_C3) = 0
                ! no grazing
                grazing_litter(:,mgraze_C3) = 2
          ...
          WHERE (sr_ugb(:,mgraze_C4) .GE. 0.0001)
            ...
            ELSEWHERE (wshtot(:,mgraze_C4) .LT. (min_grazing - 0.075) .AND. &
                      & litter_avail_totDM(:,mgraze_C4) .GE. (min_grazing + 0.05))
                ! can grazing
                ugb(:,mgraze_C4) = 1
                ! grazing litter
                grazing_litter(:,mgraze_C4) = 1
            ELSEWHERE (wshtot(:,mgraze_C4) .LT. (min_grazing - 0.075) .AND. &
                 & litter_avail_totDM(:,mgraze_C4) .LT. (min_grazing - 0.075))
                ! cannot grazing
                ugb(:,mgraze_C4) = 0
                ! no grazing
          ...
          ELSEWHERE (sr_ugb(:,mgraze_C4) .GE. 0.00002 .and. sr_ugb(:,mgraze_C4) .LT. 0.0001)
            WHERE (wshtot(:,mgraze_C4) .GE. able_grazing(:,mgraze_C4))
                ...
                grazing_litter(:,mgraze_C4) = 0
            ELSEWHERE (wshtot(:,mgraze_C4) .LT. able_grazing(:,mgraze_C4)*0.5 &
                 & .AND. litter_avail_totDM(:,mgraze_C4) .GE. able_grazing(:,mgraze_C4))
                ! can grazing
                ugb(:,mgraze_C4) = 1
                ! grazing litter
                grazing_litter(:,mgraze_C4) = 1
            ELSEWHERE (wshtot(:,mgraze_C4) .LT. able_grazing(:,mgraze_C4)*0.5 &
                 & .AND. litter_avail_totDM(:,mgraze_C4) .LT. able_grazing(:,mgraze_C4)*0.5)
                ! cannot grazing
                ugb(:,mgraze_C4) = 0
                ! no grazing
                grazing_litter(:,mgraze_C4) = 2
          ...
          WHERE (ugb(:,mgraze_C4) .EQ. 1)
              ...
              nanimaltot (:,mgraze_C4) = sr_ugb(:,mgraze_C4)
          END WHERE
! comment the snow impact, animal grazing litter during snow
!          IF (tjulian .GT. tseasonendmin) THEN
!            WHERE (snow(:) .GT. 0.0)
!              ugb(:,mgraze_C4) = 0
!            ENDWHERE
...
!ENDJCMODIF
  ...
  ELSEIF (f_autogestion .EQ. 5) THEN
    ! grazing AGB
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Animal_Init(&
    ...
    ALLOCATE (NER (npts,nvm,2),stat=ier); l_error=l_error .OR. (ier .NE. 0)   ! Net energy requirement (MJ)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Grazing_intake(&
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout) :: intakesum         ! Yearly intake per m2 (kg m-2 y-1)  
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out)   :: intake_animal     ! Daily intake per animal(kg animal-1 d-1)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout) :: intake_animalsum  ! Yearly intake per animal(kg animal-1 y-1)
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)    :: fn                ! nitrogen in structural dry matter
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)    :: n                 ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)    :: nanimaltot         ! Stocking rate (animal m-2)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out) :: intake_litter
...
! start to have intake after 5gDM/m^2
    ...
    DO j=2,nvm
      DO i=1,npts
        ...
        ! During the grazing period, wich begins at tanimal and finishes at tanimal+danimal
          IF((tjulian .GE. tanimal(i,j,h)) .AND. (tjulian .LT. (tanimal(i,j,h) + danimal(i,j,h)))) THEN
    ...
    ELSEIF (f_autogestion .EQ. 5) THEN
      ...
      WHERE (ugb(:,:) .EQ. 1 .AND. grazing_litter(:,:) .EQ. 0 &
        ...
        intake_animal = 20.0 ! 20kgDM/LSU/day for grazing biomass 
        intake = intake_animal * nanimaltot
        intake_animal_litter = 0.0
        intake_litter =0.0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Milk_Animal(&
    ...
    INTEGER(i_std)                    , INTENT(in)  :: npts
    REAL(r_std)                 , INTENT(in)  :: dt
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: nel        !nettoenergie laktation (mj/kg)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: intake_animal
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: wanimal    !lebendgewicht laktierender kuehe (kg)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot  !beweidungsdichte (gve/m**2)
    INTEGER           :: j
...
!JCMODIF for global simulation assuming no milk production
    ...
    DO j=2,nvm
      ...
      CALL Euler_funct(npts, dt, milkanimal(:,j), milkanimalsum(:,j)) !!! @equation animaux::milkanimalsum
    END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Respiration_Methane(&
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: DNDFI         ! Amount of digestible neutral detergent fiber in the intake (kg d-1)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: Wanimal       ! Animal life weight (kg)
...
!!!!!!!!!!!!   they have been moved before Respiration
  ...
  SUBROUTINE nel_grazing_calcul(&
    ...
    INTEGER(i_std)                    , INTENT(in)  :: npts                  ! r_std du domaine
    REAL(r_std)                 , INTENT(in)  :: dt                    ! pas de temps
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot             ! nombre d'animaux 
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: devstage              ! stade de développement de la pousse        
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: tgrowth               ! instant de repousse de la coupe actuelle(d)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(out) :: nel                   ! energie nette de lactation (mj/kg)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: ntot                  ! concentration en n totale (kg n/kg) 


    ! variables locales :
    REAL(r_std), DIMENSION(npts,nvm)     :: os      ! organische substanz (kg/kg)
    REAL(r_std), DIMENSION(npts,nvm)     :: rp      ! rohproteingehalt (kg/kg)
    REAL(r_std), DIMENSION(npts,nvm)     :: be      ! bruttoenergie (mj/kg)
    REAL(r_std), DIMENSION(npts,nvm)     :: vos     ! verdauliche organische substanz (kg/kg) 
    REAL(r_std), DIMENSION(npts,nvm)     :: fvos
    REAL(r_std), DIMENSION(npts,nvm)     :: vp
    REAL(r_std), DIMENSION(npts,nvm)     :: ue      ! energie métabolisable (mj/kg)
    REAL(r_std), DIMENSION(npts,nvm)     :: knel
    REAL(r_std), DIMENSION(npts,nvm)     :: rf      ! rohfasergehalt (concentration en cellulose) (kg/kg) 
  ...
  SUBROUTINE deallocation_animaux
    ...
    DEALLOCATE (milkn             , stat=ier)
    DEALLOCATE (milkc             , stat=ier)
    DEALLOCATE (ranimal           , stat=ier)
    DEALLOCATE (methane           , stat=ier)
    DEALLOCATE (faecesnsumprev    , stat=ier)
    DEALLOCATE (milkndaily        , stat=ier)
    DEALLOCATE (faecesndaily      , stat=ier)
    DEALLOCATE (urinendaily       , stat=ier)
    DEALLOCATE (milksum           , stat=ier)
    DEALLOCATE (nelgrazingsum     , stat=ier)
    DEALLOCATE (ranimalsum        , stat=ier)
    DEALLOCATE (milkcsum          , stat=ier)
    DEALLOCATE (Methanesum        , stat=ier)
    DEALLOCATE (urinecsum         , stat=ier)
    DEALLOCATE (faecescsum        , stat=ier)
    DEALLOCATE (urinensum         , stat=ier)
    DEALLOCATE (faecesnsum        , stat=ier)
    DEALLOCATE (milknsum          , stat=ier)
    DEALLOCATE (milknsumprev      , stat=ier)
    DEALLOCATE (urinensumprev     , stat=ier)
    DEALLOCATE (stockingstart     , stat=ier)
    DEALLOCATE (stockingend       , stat=ier)
    DEALLOCATE (wshtotstart       , stat=ier)
    DEALLOCATE (grazingsum        , stat=ier)
    DEALLOCATE (grazingcsum       , stat=ier)
    DEALLOCATE (grazingnsum       , stat=ier)
    DEALLOCATE (grazingc          , stat=ier)
    DEALLOCATE (grazingn          , stat=ier)
    DEALLOCATE (grazingnsumprev   , stat=ier)
    DEALLOCATE (grazingndaily     , stat=ier)
  ...
  SUBROUTINE cal_grazing(&
    ...
    INTEGER (i_std)                   , INTENT(in)  :: npts                  ! nombre de points de simulations                    
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot            ! densité de paturage (gve/m**2)
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: intake_animal                ! ingéré
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: wsh
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE chg_plante(&
    ...
    INTEGER(i_std)                                , INTENT(in)   :: npts
    REAL(r_std)                             , INTENT(in)   :: dt
    REAL(r_std), DIMENSION(npts,nvm,nparts,nelements), INTENT(inout):: biomass               ! totalité de masse sèche du shoot (kg/m2)  --> total dry mass of shoot
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: c
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: n    
    REAL(r_std), DIMENSION(npts,nvm,nleafages), INTENT(inout)       :: leaf_frac
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: wsh
    ...
    REAL(r_std), DIMENSION(npts,nvm) :: wlam           ! masse sèche (structurelle) de la limbe (kg/m2) ----> dry mass (structural) of the lamina  
    REAL(r_std), DIMENSION(npts,nvm) :: wst            ! masse sèche (structurelle) de la tige  (kg/m2) ----> dry mass (structural) of the stem
    REAL(r_std), DIMENSION(npts,nvm) :: wear            ! masse sèche (structurelle) de la tige  (kg/m2) ----> dry mass (structural) of the ear
...
! Initialisations    
    ...
    wlam(:,:) = (biomass(:,:,ileaf,icarbon)/(1000*CtoDM))     /(1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )      ! leaf dry mass 
    wst(:,:)  = (biomass(:,:,isapabove,icarbon)/(1000*CtoDM)) /(1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )      ! stem dry mass
    wear(:,:) = (biomass(:,:,ifruit,icarbon)/(1000*CtoDM)) /(1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )      ! ear dry mass
...
!print *, 'PlantLam'
    WHERE ((wst .GT. 0.0) .AND. (MassePondTot .GT. 0.0) .AND. (grazingstruct .GT. 0.0))
...
!print *,'after frac'
   ...
   ! when grazing litter, now assumed to be without trampling
    WHERE (nanimaltot(:,:) .GT. 0.0 .AND. grazing_litter(:,:) .EQ. 0 )
...
!       trampling(:,:) = nanimaltot * 10000 * 0.008 *(biomass(:,:,ileaf)+biomass(:,:,isapabove)+biomass(:,:,ifruit))
    ...
    ENDWHERE
        ...
        biomass(:,:,ileaf,icarbon)     = (wlam(:,:) * 1000*CtoDM) * (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
        biomass(:,:,isapabove,icarbon) = (wst(:,:)  * 1000*CtoDM) * (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
        biomass(:,:,ifruit,icarbon)    = (wear(:,:)  * 1000*CtoDM) * (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE variablesPlantes(&
    ...
    INTEGER(i_std)                                , INTENT(in)   :: npts
    REAL(r_std), DIMENSION(npts,nvm,nparts,nelements), INTENT(in):: biomass ! totalité de masse sèche du shoot (kg/m2)  --> total dry mass of shoot
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: c
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)     :: n  
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: intake_animal
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)  :: intakemax
    ...
    WHERE ((ABS(wlam+wst) .GT. 10e-15) .AND. (intake_animal .GT. 0.0))
        ...
        exposant_stem = -2. * betagrazing * MAX(0.,1.-(intakemax - intake_animal))*(NDFmean - NDFstem )*100.

        pstem = 1./(ncomp)*((1. - 2.*LimDiscremine)*(1. - exp(exposant_stem))/ &
           (1. + EXP(exposant_stem))+1.)

        ! # for the lam compartment
        exposant_lam = -2.*betagrazing *MAX(0.,1.-(intakemax - intake_animal))*(NDFmean - NDFlam)*100.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Animaux_main_dynamic(&
    ...
    INTEGER(i_std), INTENT(in)                                    :: npts                ! Number of spatial points (-)
    REAL(r_std ), INTENT(in)                               :: dt                  ! Time step (d)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: devstage            ! Developmental stage (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)           :: intakemax                   ! intake capacity of the cattle (kg/(animal*m**2)
    REAL(r_std ), DIMENSION(npts), INTENT(in)              :: snow                ! Snow cover (mm)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: wshtot              ! Total (structure + substrate) shoot dry matter(kg m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: wsh              ! (structure + substrate) shoot dry matter(kg m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)             :: nel                 ! Net energy content of the forage (MJ kg-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)           :: nanimaltot              ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)             :: intake              ! intake (kg DM m2-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)              :: import_yield        ! ajout de Nicolas pour les runs saturant nonlimitant
    LOGICAL, INTENT(in)                                    :: new_year
    LOGICAL, INTENT(in)                                    :: new_day
    INTEGER(i_std), INTENT(in)                                    :: tcutmodel
    INTEGER(i_std ), INTENT(in)                               :: tjulian             ! Julian day (-)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: nanimal             ! Stocking density  h (1,..,nstocking) (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: tanimal             ! Beginning of the grazing period    h (1,..,nstocking) (d)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: danimal             ! Lenght of the grazing period    h (1,..,nstocking) (d)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PIYcow              ! Initial weight of Young cow (Kg)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PIMcow              ! Initial weight of Mature cow (Kg)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: BCSYcow             ! Initial body score condition of Young cow(Kg)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: BCSMcow             ! Initial body score condition of mature cow(Kg)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PICcow              ! Initial weight of cow's calves (Kg)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: AGE_cow_P           ! Average age of dairy primiparous cows for autogestion
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(inout) :: AGE_cow_M           ! Average age of dairy multiparous cows for autogestion
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)           :: intakesum           ! Yearly intake (kg animal-1 y-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)           :: intakensum          ! N in daily intake per m2(kgN/m2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: fn                  ! nitrogen in structural dry matter
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: ntot                ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: c
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: n                   ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std ), DIMENSION(npts,nvm,nleafages), INTENT(inout)              :: leaf_frac
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)             :: intake_animal       ! Daily intake per animal(kg animal-1 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)           :: intake_animalsum    ! Yearly intake per animal(kg animal-1 d-1)
    REAL(r_std ), DIMENSION(npts), INTENT(in)              :: tadmin              ! Daily minimum temperature
    REAL(r_std ), DIMENSION(npts), INTENT(in)              :: tadmoy              ! Daily average temperature (K)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)             :: IC_tot              ! Daily average ingested capacity of cows (kg)
    REAL(r_std ), DIMENSION(npts,nvm,n_out),INTENT(out)        :: Autogestion_out     ! Fraction F (npts,1), ratio F (npts,2), and lenght of the grazing period when autgestion
                                                                                  ! To write in import_yiels File(npts,3)
    INTEGER(i_std),                       INTENT(in)              :: type_animal         ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    REAL(r_std ), DIMENSION(npts,nvm,nstocking),INTENT(inout)  :: Forage_quantity     ! Net energy ingested for cow (young in first, and adult in second) (MJ)
    REAL(r_std ), DIMENSION(npts),  INTENT(in)             :: tmoy_14             ! 14 day running average of daily air temperature (K)
    REAL(r_std ),                   INTENT(in)             :: intake_tolerance    ! intake tolerance threshold (-)
    REAL(r_std ),                   INTENT(in)             :: q_max_complement    ! Maximum quantity of forage or concentrate to supplement animals when auto-supplementation (kg)
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(inout):: biomass       ! totalité de masse sèche du shoot(kg/m**
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: urinen      ! n dans l'urine (kg n /(m**2 d))     
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: faecesn     ! n dans les fèces (kg n /(m**2*d)) 
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: urinec      ! c dans les urines
    REAL(r_std), DIMENSION(npts,nvm)          , INTENT(out)   :: faecesc     ! c dans les fèces (kg c /(m**2*d))
    ...
    REAL(r_std )     , DIMENSION(npts,nvm)  :: wshtotgrazing  ! Grazing shoot biomass (kg DM m-2)
    REAL(r_std )     , DIMENSION(npts,nvm)  :: deltaanimal
    REAL(r_std )     , DIMENSION(npts,nvm)  :: extra_feed     ! Forage necessary to feed animals at barn when stocking rate autogestion (kg DM m-2)
    REAL(r_std )     , DIMENSION(npts,nvm)  :: nb_ani_old     ! Actual stocking rate per ha of total pasture "D" at previous iteration (animal (ha of total grassland)-1)
    INTEGER(i_std)          , DIMENSION(npts,nvm)  :: ugb_last       ! Equals 0 (no animals) or 1 (animals) for console display

    REAL(r_std ), DIMENSION(npts,nvm)              :: OMD                 ! Digestible organic matter in the intake(kg/kg)
    REAL(r_std ), DIMENSION(npts,nvm,2)            :: NEIcow              ! Total net energy intake (1:young, 2:adult) (MJ)
    ! to check
    REAL(r_std ), DIMENSION(npts,nvm,2)            :: NEIh                ! Net energy intake from the ingested herbage(1:young, 2:adult) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)            :: NEIf                ! Net energy intake from the ingested forage(1:young, 2:adult) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)            :: NEIc                ! Net energy intake from the ingested concentrate(1:young, 2:adult) (MJ)

    !milk
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: MPwcow2          ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: MPcow2           ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: MPcow            ! Daily milk production per m2 for primiparous or multiparous cows (kg/m-2/d)
    REAL(r_std ), DIMENSION(npts,nvm)       :: milkKG           ! Daily actual milk production per animal for the whole cattle (kg/animal/d)

    !intake capacity and DMI
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: ICcow            ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: DMIcowanimal     ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: DMIcow           ! Daily intake per m2 for primiparous or multiparous cows(kg/m2/d)
    REAL(r_std ), DIMENSION(npts,nvm)       :: ICcalf           ! Calf intake capacity  (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm)       :: DMIcalfanimal    ! Daily calf intake per animal(kg/animal/d)         
    REAL(r_std ), DIMENSION(npts,nvm)       :: DMIcalf          ! Daily calf intake per m2 (Kg/d)         

    !Energie Balance
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NELherbage      ! Energetic content of the herbage (MJ/kg) 
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEPcow          ! Net energy for production (young :1 , adult:2) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEPlactcow      ! Net energy for milk production (young :1 , adult:2) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEPgestcow      ! Net energy for gestation (suckler cows)(young :1 , adult:2) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEMcow          ! Net energy for maintenance (young :1 , adult:2) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEBcow          ! Net energy Balance (young :1 , adult:2) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm,2)     ::  NEGcow          ! Net energy for gestation (dairy cows)(young :1 , adult:2) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NEIcalf         ! Net energy intake for calves (from milk and ingested herbage) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NEIherbagecalf  ! Net energy intake for calves (from only ingested herbage) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NEImilkcalf     ! Net energy intake for calves (from only ingested milk) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NEGcalf         ! Net energy for calf growth (MJ)
    REAL(r_std ), DIMENSION(npts,nvm)       ::  NEMcalf         ! Net energy for calf maintenance (MJ)
    !BILAN N C    
    REAL(r_std ), DIMENSION(npts,nvm)       ::  faecesNcow      ! Nitrogen in faeces (young in first, and adult in second)(Kg N m-2)   
    REAL(r_std ), DIMENSIOn(npts,nvm)       ::  faecesCcow      ! Carbon in faeces (young in first, and adult in second)(Kg C m-2)
    REAL(r_std ), DIMENSIOn(npts,nvm)       ::  urineNcow       ! Nitrogen in urine (young in first, and adult in second)(Kg N m-2)
    REAL(r_std ), DIMENSIOn(npts,nvm)       ::  urineCcow       ! Carbon in Urine (young in first, and adult in second)(Kg C m-2)
    REAL(r_std ), DIMENSION(npts,nvm)       :: nWeekLact        ! Lactation week (in weeks from calving)
    REAL(r_std ), DIMENSION(npts,nvm)       :: nweekGest        ! Gestation week (in weeks from mating)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: AGE_animal       ! Animal age in case of simulation of dairy cows (months)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: CH4h             ! Daily enteric methane production from ingested herbage  (kg C animal-1 d-1)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: deltaBCS         ! Body condition score variation between two consecutive time steps (-) 
    INTEGER(i_std), DIMENSION(npts,nvm)            :: in_grazing
    INTEGER(i_std)                             :: i,j               ! For loop
    REAL(selected_real_kind(3,2))       :: tempTjulian       ! TO round Tjulian

    REAL(r_std ),DIMENSION(npts,nvm)        :: FVh               ! Herbage Fill Value (UE)
    REAL(r_std ), DIMENSION(npts,nvm,2)     :: MPpos             ! Possible milk production of dairy cows according to the diet (kg/animal/d)   

    REAL(r_std), DIMENSION(npts,nvm)       ::  WanimalMOYcow   ! The average weigth of live of the cattle (Kg / animal)

    REAL(r_std), DIMENSION(npts,nvm,2)     ::  CH4animal             ! Daily enteric methane production from ingested herbage  (kg C animal-1 d-1)
            ...
            IF (f_postauto .EQ. 1) THEN
                ...
                WHERE ((ok_ugb(:,mgraze_C4) .EQ. 0))
                  ...
                  WHERE (grazed_frac(:,mgraze_C4) .GT. 0.7)
                    ...
                    grazed_frac(:,mgraze_C4)=nb_ani(:,mgraze_C4)/sr_ugb(:,mgraze_C4)
                  END WHERE
                    Local_autogestion_out(:,mgraze_C4,1)=extra_feed(:,mgraze_C4)/(import_yield(:,mgraze_C4) * 0.85)
    ...
    n_day : IF (new_day .EQV. .TRUE. ) THEN
        ...
        DO j=2,nvm
          IF (is_grassland_manag(j) .AND. (.NOT.is_grassland_cut(j)).AND.(.NOT.is_grassland_grazed(j)))THEN
           ...
           DO k=1,nstocking
             DO i=1,npts
               IF (tanimal(i,j,k).EQ. tjulian .AND.f_autogestion.NE.2 .AND. f_postauto .NE. 1) THEN
             ...
             IF (( wshtot(i,j).GT.BM_threshold+0.05) .AND.f_autogestion.NE.2 .AND.f_postauto .NE. 1 &
               .AND. (tjulian .GE. tanimal(i,j,k)) .AND. (tjulian .LT. (tanimal(i,j,k) + danimal(i,j,k))) ) THEN
                 nanimaltot(i,j)  =nanimal(i,j,k)
             ENDIF
          ENDDO ! npts
...
! (à defaut 15kg MS/UGB/m2)
        ...
        calc_nanimaltot : IF (tcutmodel .EQ. 1) THEN
          DO j=2,nvm
            ...
            WHERE ((nanimal(:,j,1) .GT. 0.0) .AND. (devstage(:,j) .GT. devstocking) .AND. (stockingstart(:,j) .EQ. 0))
...
!ENDJCADD
    ...
    ! simulation
        ...
        DO j=2,nvm
          IF (is_grassland_manag(j) .AND. (.NOT.is_grassland_cut(j)).AND.(.NOT.is_grassland_grazed(j)))THEN
            ...
            DO i=1,npts
             ...
             IF((nanimaltot_prec(i,j).EQ.0).AND.(nanimaltot(i,j).NE.0).AND.(compte_pature(i,j).LE.10)) THEN      
      ...
      ELSE IF (f_postauto.EQ.1 .OR. f_postauto .EQ. 5) THEN
         DO i=1,npts
             ...
             !les données pour ecriture management
             IF((nanimaltot_prec(i,mgraze_C3).EQ.0).AND.(nanimaltot(i,mgraze_C3).NE.0).AND.(compte_pature(i,mgraze_C3).LE.10))THEN
                 !nous sommes limites à 10 periodes de paturage
             ...
             IF((nanimaltot_prec(i,mgraze_C4).EQ.0).AND.(nanimaltot(i,mgraze_C4).NE.0).AND.(compte_pature(i,mgraze_C4).LE.10))THEN
                 !nous sommes limites à 10 periodes de paturage
             ...
             ENDIF
             !cas d'une sortie de paturage
             IF(nanimaltot_prec(i,mgraze_C4).NE.0.AND.nanimaltot(i,mgraze_C4).EQ.0)THEN
                 print *, "compte pature : ", compte_pature(i,mgraze_C4)
    ...
    DO j=2,nvm
     ...
     !84 est 365 moins la durée de gestation(280j) 
          ...
          ENDIF
        ELSE
          IF (tempTjulian+365-tcalving(i,j) .GE. 84 .and. tempTjulian+365-tcalving(i,j) .LE. 365) THEN
            gestation(i,j)=1
          ENDIF
   ...
   IF(type_animal.EQ.2) THEN
    DO j=2,nvm
      IF (f_autogestion.EQ.2) THEN
            DO i=1,npts
                IF (nanimaltot_prec(i,j).EQ.0.AND.nanimaltot(i,j).GT.0.AND.calf(i,j).EQ.1) THEN
...
!--------------------------------------------------------
   !
    WHERE(nanimaltot.GT.0.0.AND.MPcow2(:,:,1).GT.0.0.AND.type_animal.eq.1.AND.f_complementation.EQ.4)
       Qic(:,:,1)=DMIc(:,:,1)/MPcow2(:,:,1)
    ENDWHERE

    WHERE(nanimaltot.GT.0.0.AND.MPcow2(:,:,2).GT.0.0.AND.type_animal.eq.1.AND.f_complementation.EQ.4)
...
!------------------------
    ...
    ELSEIF(type_animal.EQ.2) THEN
      ...
      CALL balance_energy_cow(npts,dt          ,&
         ...
         EVc(:,:),Qic(:,:,1), NEIcow(:,:,1), NEIh(:,:,1), NEIf(:,:,1), NEIc(:,:,1),& ! to check
      ...
      CALL balance_energy_cow(npts,dt          ,&
         ...
         EVc(:,:),Qic(:,:,2), NEIcow(:,:,2), NEIh(:,:,2), NEIf(:,:,2), NEIc(:,:,2),& ! to check
  ...
  !!!!!! So we call modified cal_grazing which from MODULE applic_plant to get variables needed
    CALL cal_grazing(&
     ...
     DO j=2,nvm
       ...
       CALL Euler_funct (npts, dt, grazingc(:,j), grazingcsum(:,j))
     ENDDO
       WanimalMOYcow = (Wanimalcow(:,:,1)*pyoung + wanimalcow(:,:,2)*(1-pyoung) + wanimalcalf)
      ...
      ENDIF
       ...
       ENDWHERE
     DO j=2,nvm
       CALL Euler_funct (npts, dt, ranimal(:,j), ranimalsum(:,j)) !!! @equation animaux::ranimalsum
       CALL Euler_funct (npts, dt, methane(:,j), Methanesum(:,j)) !!! @equation animaux::Methanesum
       CALL Euler_funct (npts, dt, methane_ani(:,j), Methane_aniSum(:,j))!!! @equation animaux::Methane_aniSum
     ENDDO
      !------------------
      !Excreta  
      !------------------
...
!
  ...
  !*suckler Cow
   SUBROUTINE intake_capacity_cow( &
      ...
      REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: wanimalcow          ! Animal liveweight (kg/animal) (young:1, adult:2)
      REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2             ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
      REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: BCScow              ! Body score condition cow (young in first, and adult in second) (/5)
      REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot          ! Stocking rate (animal m-2)
      REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: ICcow               ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
  ...
  ! Suckler Calf
   ...
   SUBROUTINE intake_capacity_calves(&
      ...
      INTEGER, INTENT(in)                               :: npts                ! Number of spatial points (-)
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: wanimalcalf         ! Calf liveweigth (kg/animal)
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: nanimaltot          ! Stocking rate (animal m-2)
      INTEGER(i_std ), INTENT(in)                          :: tjulian             ! Julian day (-) 
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)        :: ICcalf              ! Calf intake capacity  (kg/animal/d)

      INTEGER, DIMENSION(npts,nvm)                          :: dsevrage            ! Julian day of the suckling calf period
  ...
  SUBROUTINE intake_capacity_cow_d(&
    ...
    INTEGER, INTENT(in)                               :: npts                ! Number of spatial points (-)
    INTEGER, INTENT(in)                               :: npta                ! equal 2 when cow (Young and old) and 1 when calf
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2             ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: BCS                 ! Body Condition Score (for cow only /5)
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: wanimalcow          ! Animal liveweight (kg/animal) (young:1, adult:2)
    REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: nanimaltot          ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: IC_animal           ! intake Capacity (Kg)
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: AGE_animal          ! Animal age in case of simulation of dairy cows (months)
    REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: nWeekLact           ! Lactation week (in weeks from calving) 
    REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: nWeekGest           ! Gestation week (in weeks from mating)
         
    REAl(r_std ),DIMENSION(npts,nvm,npta)                 :: IL                  ! Lactation Index
    REAL(r_std ),DIMENSION(npts,nvm)                      :: IG                  ! Gestation Index
    REAL(r_std ),DIMENSION(npts,nvm,npta)                 :: IM                  ! Maturity Index
    ...
    Ic_animal(:,:,1)= (13.9+(0.015*(Wanimalcow(:,:,1)-600))+(0.15*MPwcow2(:,:,1))+(1.5*(3-BCS(:,:,1))))*IL(:,:,1)*IG*IM(:,:,1)   
    Ic_animal(:,:,2)= (13.9+(0.015*(Wanimalcow(:,:,2)-600))+(0.15*MPwcow2(:,:,2))+(1.5*(3-BCS(:,:,2))))*IL(:,:,2)*IG*IM(:,:,2)   
  ...
  SUBROUTINE intake_capacity_heifer(&
    ...
    INTEGER, INTENT(in)                              :: npts        ! Number of spatial points (-)
    INTEGER, INTENT(in)                              :: type_animal ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)        :: winit       ! Initial live weigth of heifer
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)        :: wanimalcow  ! Animal liveweight (kg/animal) (young:1, adult:2) 
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)       :: IC_animal   ! Heifer intake capacity
                
    ! variable local            
    REAL(r_std ), DIMENSION(npts,nvm)                    :: P1          ! Parameter for IC calculation 
    REAL(r_std ), DIMENSION(npts,nvm)                    :: itype       ! Parameter for IC calculation  
  ...
  SUBROUTINE Grazing_intake_cow(&
    ...
    INTEGER, INTENT(in)                          :: npts                   ! Number of spatial points (-)
    INTEGER, INTENT(in)                          :: type_animal            ! 1 or 2 or 4 or 5= > new module animal
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)    :: wshtot                 ! Shoot structural dry matter (kg m-2)
    REAL(r_std ), DIMENSION(npts), INTENT(in)    :: tadmin                 ! Daily minimum temperature
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout) :: nanimaltot 	           ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)    :: DNDF                   ! fraction of digestible fibres in total fibres (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)    :: NDF                    ! fraction of fibres in the intake(-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)    :: IC                     ! intake capacity (Kg)    
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)   :: DMIanimal              ! Dry Matter intake of a cow/calf (Kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)   :: OMD                    ! Digestible organic matter in the intake(kg/kg)
    REAL(r_std ), DIMENSION(npts), INTENT(in)    :: tadmoy                 ! Daily average temperature (K)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)   :: FVh                    ! Herbage Fill Value (UE)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)    :: ntot                   ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std ), DIMENSION(npts), INTENT(in)    :: tmoy_14                ! 14 day running average of daily air temperature (K)
    REAL(r_std ),                  INTENT(in)    :: BM_threshold           ! Biomass threshold above which animals are moved out the paddock (kg/m2)
    !implicit variables intent(in) :
    ! - AnimalqintakeM : intake parameter (m2 m-2)
  
    !Local variables
    INTEGER                                       :: i,j
    REAL(r_std ), DIMENSION(npts,nvm)                 :: NDFnd                  ! fraction of non digestible fibres in the intake(g/Kg)
    REAL(r_std ), DIMENSION(npts)                 :: temperature_effect_OMD ! temperature effect on organic matter digestibility (-)
...
!         IF(((wshtot(i,j).GT.BM_threshold).OR.f_complementation.EQ.4).and.(nanimaltot(i,j).NE.0)) THEN      
         WHERE(((wshtot(:,j).GT.able_grazing(:,j)).OR.f_complementation.EQ.4).and.(nanimaltot(:,j).NE.0))
!ENDJCMODIF
...
!     ENDDO
  ...
  SUBROUTINE Grazing_intake_cow_d(&
    ...
    INTEGER, INTENT(in)                                 :: npts          ! Number of spatial points (-)
    INTEGER, INTENT(in)                                 :: npta          ! equal 2 for primi and multipare
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)           :: wshtot        ! Shoot structural dry matter (kg m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)           :: ntot          ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)        :: nanimaltot 	 ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)           :: DNDF          ! fraction of digestible fibres in total fibres (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)           :: NDF           ! fraction of fibres in the intake(-)
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)      :: IC            ! intake capacity (Kg)    
    REAL(r_std ), DIMENSION(npts), INTENT(in)           :: tadmin        ! Daily minimum temperature
    REAL(r_std ), DIMENSION(npts), INTENT(in)           :: tadmoy        ! Daily average temperature
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)     :: DMIanimal     ! Dry Matter intake of a cow/calf (Kg)
    ...
    REAL(r_std ),DIMENSION(npts,nvm)                        :: NDFnd         ! fraction of non digestible fibres in the intake(g/Kg)
    !06/02/2010 AIG & MG
    LOGICAL,DIMENSION(npts,nvm)                             :: Bool_movedout ! Bolean to decide to move out animal

    INTEGER                                             :: i,j
       
    REAL(r_std ),DIMENSION(npts)                        :: temperature_effect     ! temperature effect on dry matter intake (-)
    REAL(r_std ),DIMENSION(npts)                        :: temperature_effect_OMD ! temperature effect on organic matter digestibility (-)
...
!     WHERE((nanimaltot(:,j).NE.0).AND.((wshtot(:,j).GT.BM_threshold).OR.(f_complementation.EQ.4)))
     WHERE((nanimaltot(:,j).NE.0).AND.((wshtot(:,j).GT.able_grazing(:,j)).OR.(f_complementation.EQ.4)))
!ENDJCMODIF 
     !WHERE(nanimaltot.NE.0)  
...
!    ENDDO
  ...
  INTEGER, INTENT(in)                               :: npts                    ! Number of spatial points (-)
  REAL(r_std ), INTENT(in)                          :: dt                      ! Time step (d)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowanimal            ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: FVh                     ! Herbage Fill Value (UE)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: ICcow                   ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: FVf                     ! forage fill value (Kg)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2                 ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(inout)    :: forage_quantity_period  ! Daily forage quantity provided to herbivors during the current stocking period (Kg/Animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,2)  , INTENT(inout)    :: QIc                     ! Daily concentrate quantity per kg of milk or per kg of lw
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: EVf                     ! Energetic content of the forage (MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot              ! Stocking rate (animal/m²)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowsum               ! Cumulated intake per m2 for primiparous or multiparous cows(kg/m2)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowanimalsum         ! Cumulated animal intake for primiparous or multiparous cows(kg/animal)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIcow                  ! Daily intake per m2 for primiparous or multiparous cows(kg/m2/d)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIcowNsum              ! N in daily intake per m2 for primiparous or multiparous cows(kgN/m2)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: n                       ! nitrogen substrate concentration in plant,(kg n/kg)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: fn                      ! nitrogen in structural dry matter
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: pyoung                  ! Fraction of young or primiparous in the cattle (-)
  INTEGER                        , INTENT(in)       :: type_animal             ! kind of herbivores (1: dairy cows, 2 suckler cows+calf, 3 old module, 4 dairy heifers, 5 suckler heifers)
  REAL(r_std )                   , INTENT(in)       :: intake_tolerance        ! intake tolerance threshold (-)
  REAL(r_std )                   , INTENT(in)       :: Q_max_complement        ! Maximum quantity of forage or concentrate to supplement animals when auto-supplementation (kg)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: NER                     ! Net energy requirement (MJ)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)      :: forage_complementc      ! fraction of carbon in Forage + concentrate (kgC/m²/d)
  REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)      :: forage_complementn      ! fraction of nitrogen in Forage + concentrate (kgC/m²/d)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: NEI                     ! Net energy intake(MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: NEM                     ! Net energy requirements for maintenance(MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIh                    ! Net Energy intake from ingested herbage(MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIf                    ! Net Energy intake from ingested forage(MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIc                    ! Net Energy intake from ingested concentrate(MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: NEG                     ! Net energy required for gestation (MJ)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIc                    ! Concentrate intake (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIf                    ! forage intake (kg/animal/d)
  
  !local variables
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Shf                     ! substitution rate of herbage by forage in the cow diet (-)
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Shc1                    ! substitution rate of herbage by concentrate in the cow diet (-)
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Shc2                    ! substitution rate of herbage by concentrate in the cow diet (-)
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Shfc                    ! substitution rate of herbage by concentrate in the cow diet (-)
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: EDhf                    ! substitution rate of herbage by concentrate in the cow diet (-)
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: A                       ! intermediary variable
  
  REAL(r_std ), DIMENSION(npts,nvm)                     :: ICmoy                   ! Average intake capacity of the cattle [kg MS/animal/d]
  REAL(r_std ), DIMENSION(npts,nvm)                     :: DMImoy                  ! Average dry matter intake of the cattle [kg MS/animal/d] 
  
  REAL(r_std ), DIMENSION(npts,nvm,2)                   :: temp                    ! temporary variable
  INTEGER, INTENT(in)                               :: f_complementation       ! Flag to activate cow supplementation
  ...
  DMIf=0.0
  DO j=2,nvm
    IF(f_complementation.EQ.1.OR.f_complementation.EQ.3) THEN !supplementation with forage only or with forage and concentrate
     ...
     IF(f_complementation.EQ.3) THEN !supplementation with forage and concentrate
      DO i=1,npts
           ...
           IF(nanimaltot(i,j).GT.0) THEN       
                 ...
                 EDhf(i,j,k)=(DMIcowanimal(i,j,k)*NELherbage(i,j)/7.12+Forage_quantity_period(i,j)*EVf(i,j))/(DMIcowanimal(i,j,k)*FVh(i,j)+Forage_quantity_period(i,j)*FVf(i,j))
                 A(i,j,k)=(0.0004*MPwcow2(i,j,k)**2)+(2.39*(EDhf(i,j,k))**2)-(0.0452*MPwcow2(i,j,k)*(EDhf(i,j,k)))         
                 Shfc(i,j,k)=0.11+(0.02*DMIc(i,j,k))-(1.13*(EDhf(i,j,k))**2)+A(i,j,k)*((DMIcowanimal(i,j,k)*FVh(i,j)+Forage_quantity_period(i,j)*FVf(i,j))/ICcow(i,j,k))
    ...
    ENDIF
     DO i=1,npts
       DO k=1,2
           IF(nanimaltot(i,j).GT.0) THEN
               Shf(i,j,k)=((DMIcowanimal(i,j,k)*FVh(i,j))/ICcow(i,j,k))*(2.2-1.2*(FVh(i,j)/FVf(i,j)))
               DMIcowanimal(i,j,k)=DMIcowanimal(i,j,k)-Shf(i,j,k)*Forage_quantity_period(i,j)
          ...
          DO i=1,npts
            ...
            DO k=1,2
                IF(nanimaltot(i,j).GT.0) THEN
                     !DMIc(i,j)=QIc(i)*MPcow2(i,j)
                     DMIc(i,j,k)=QIc(i,j,k)*MPwcow2(i,j,k)
                     A(i,j,k)=(0.0004*MPwcow2(i,j,k)**2)+(2.39*(NELherbage(i,j)/(7.12*FVh(i,j)))**2)-(0.0452*MPwcow2(i,j,k)*(NELherbage(i,j)/(7.12*FVh(i,j))))
                     Shc1(i,j,k)=0.8+0.01*DMIc(i,j,k)
                     shc2(i,j,k)=0.11+(0.02*DMIc(i,j,k))-(1.13*(NELherbage(i,j)/(7.12*FVh(i,j)))**2)+A(i,j,k)*((DMIcowanimal(i,j,k)*FVh(i,j))/ICcow(i,j,k))
                     DMIcowanimal(i,j,k)=DMIcowanimal(i,j,k)-min(Shc1(i,j,k),Shc2(i,j,k))*DMIc(i,j,k)                                     
                ENDIF                       
            ENDDO
          ENDDO  
  ...
  DO j=2,nvm 
   ...
   WHERE(nanimaltot(:,j).GT.0.AND.f_complementation.LT.4) 
       forage_complementc(:,j)=0.60*((forage_quantity_period(:,j)+DMIc(:,j,1))*pyoung(:,j) + (forage_quantity_period(:,j)+DMIc(:,j,2))*(1-pyoung(:,j)))*nanimaltot(:,j)
       forage_complementn(:,j)=((fN_forage(:,j)*forage_quantity_period(:,j)+fN_concentrate(:,j)*DMIc(:,j,1))*pyoung(:,j) &
                           + (fN_forage(:,j)*forage_quantity_period(:,j)+fN_concentrate(:,j)*DMIc(:,j,2))*(1-pyoung(:,j)))*nanimaltot(:,j)
   ELSEWHERE(nanimaltot(:,j).GT.0.AND.f_complementation.EQ.4)                    
           forage_complementc(:,j)=0.60*((DMIf(:,j,1)+DMIc(:,j,1))*pyoung(:,j) + (DMIF(:,j,2)+DMIc(:,j,2))*(1-pyoung(:,j)))*nanimaltot(:,j)
           forage_complementn(:,j)=((fN_forage(:,j)*DMIf(:,j,1)+fN_concentrate(:,j)*DMIc(:,j,1))*pyoung(:,j) &
                           + (fN_forage(:,j)*DMIf(:,j,2)+fN_concentrate(:,j)*DMIc(:,j,2))*(1-pyoung(:,j)))*nanimaltot(:,j)
   ELSEWHERE    
  ...
  SUBROUTINE auto_complementation_dairy(npts,DMIcowanimal,FVh,ICcow,NER,NELherbage, EVc,&
     ...
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowanimal            ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: FVh                     ! Herbage Fill Value (UE)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: ICcow                   ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: NER                     ! Net energy requirement (MJ)
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: EVc                     ! Energetic value of the forage  (MJ/kg) 
     REAL(r_std )                   , INTENT(in)       :: Q_max_complement        ! Maximum quantity of forage or concentrate to supplement animals when auto-supplementation (kg)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIc                    ! Forage quantity calculated by the model (kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2                 ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: NEI                     ! Net energy intake(MJ)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: NEM                     ! Net energy requirements for maintenance (MJ)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: NEIh                    ! Net Energy intake from ingested herbage(MJ)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: NEIf                    ! Net Energy intake from ingested forage(MJ)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: NEIc                    ! Net Energy intake from ingested concentrate(MJ)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: NEG                     ! Net energy required for gestation
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot              ! Stocking rate (animal/m²)

     
     !local variables
     REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Shc1,shc2,shc           ! Substitution rate of herbage by concentrate in the cow diet (-)
     REAL(r_std ), DIMENSION(npts,nvm,2)                   :: A                       ! Intermediary variable
     REAL(r_std ), DIMENSION(npts,nvm,2)                   :: MPpos_loc               ! Possible milk production (local) (Kg/UGB)
     REAL(r_std ), DIMENSION(npts,nvm,2)                   :: Qic                     ! Quantité de concentré ingéré par Kg de lait
     REAL(r_std ), DIMENSION(npts,nvm)                     :: EDh                     ! Substitution rate of herbage by concentrate in the cow diet (-)
     REAL(r_std ), DIMENSION(npts,nvm)                     :: temp                    ! Intermediairy variable
     LOGICAL,      DIMENSION(npts,nvm,2)                   :: fin                     ! To stop the iterative algorithm
     REAL(r_std ), DIMENSION(npts,nvm)                     :: ICmoy                   ! Average intake capacity of the cattle [kg MS/animal/d]
     REAL(r_std ), DIMENSION(npts,nvm)                     :: DMImoy                  ! Average dry matter intake of the cattle [kg MS/animal/d] 
     INTEGER     , DIMENSION(npts,nvm)                     :: Loop_count              ! Counter for loop               
     ...
     DO WHILE(NOT(ALL(fin)))  
         ...
         A(:,:,1)=(0.0004*MPcow2(:,:,1)**2)+(2.39*EDh(:,:)**2)-(0.0452*MPwcow2(:,:,1)*EDh(:,:))
         A(:,:,2)=(0.0004*MPcow2(:,:,2)**2)+(2.39*EDh(:,:)**2)-(0.0452*MPwcow2(:,:,2)*EDh(:,:))
         shc1(:,:,1)=0.8+0.01*DMIc(:,:,1)
         shc1(:,:,2)=0.8+0.01*DMIc(:,:,2)
         shc2(:,:,1)=0.11+(0.02*DMIc(:,:,1))-(1.13*EDh(:,:)**2)+A(:,:,1)*(DMIcowanimal(:,:,1)*FVh/Iccow(:,:,1))
         shc2(:,:,2)=0.11+(0.02*DMIc(:,:,2))-(1.13*EDh(:,:)**2)+A(:,:,2)*(DMIcowanimal(:,:,2)*FVh/Iccow(:,:,2))
         
         shc(:,:,1)=min(shc1(:,:,1),shc2(:,:,1))
         shc(:,:,2)=min(shc1(:,:,2),shc2(:,:,2))
         
         WHERE(.NOT.(fin(:,:,1)))
            DMIc(:,:,1)=(NER(:,:,1)-DMIcowanimal(:,:,1)*NELherbage(:,:))/(7.12*EVc(:,:)-shc(:,:,1)*NELherbage(:,:))
         ENDWHERE
         
         WHERE(.NOT.(fin(:,:,2)))
            DMIc(:,:,2)=(NER(:,:,2)-DMIcowanimal(:,:,2)*NELherbage(:,:))/(7.12*EVc(:,:)-shc(:,:,2)*NELherbage(:,:))
         ENDWHERE
         
         WHERE(((NER(:,:,1)-DMIcowanimal(:,:,1)*NELherbage(:,:)).LT.0.0).OR.((7.12*EVc(:,:)-shc(:,:,1)*NELherbage(:,:)).LT.0.0))
            DMIc(:,:,1)=0.0
         ENDWHERE
         
         WHERE(((NER(:,:,2)-DMIcowanimal(:,:,2)*NELherbage(:,:)).LT.0.0).OR.((7.12*EVc(:,:)-shc(:,:,2)*NELherbage(:,:)).LT.0.0))
            DMIc(:,:,2)=0.0
         ENDWHERE        
  ...
  SUBROUTINE  auto_complementation_suckler(npts,DMIcowanimal,FVh,ICcow,NER,NELherbage, &
     ...
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: NER                     ! Net energy requirement (MJ)
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: EVf                     ! Energetic value of the forage  (MJ/kg) 
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: FVf                     ! Forage vill value  (UE) 
     REAL(r_std )                   , INTENT(in)       :: Q_max_complement        ! Maximum quantity of forage or concentrate to supplement animals when auto-supplementation (kg)
     REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)      :: DMIf                    ! Forage quantity calculated by the model (kg/animal/d)
     REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot              ! Stocking rate (animal/m²)
     REAL(r_std )                   , INTENT(in)       :: intake_tolerance        ! intake tolerance threshold (-)

     !local variables
     REAL(r_std ), DIMENSION(npts,nvm)                     :: Shf                     ! Substitution rate of herbage by forage in the cow diet (-)
     REAL(r_std ), DIMENSION(npts,nvm)                     :: ICmoy                   ! Average intake capacity of the cattle [Kg MS/UGB]
     REAL(r_std ), DIMENSION(npts,nvm)                     :: DMImoy                  ! Average dry matter intake of tje cattle [Kg MS/UGB] 
     ...
     WHERE(nanimaltot(:,:).GT.0.0)
        ...
        Shf(:,:)= ((DMIcowanimal(:,:,1)*FVh(:,:))/ICcow(:,:,1))*(2.2-1.2*FVh(:,:)/FVf(:,:))
        
        DMIf(:,:,1)=(NER(:,:,1)-DMIcowanimal(:,:,1)*NELherbage(:,:))/(7.12*EVf(:,:)-SHf(:,:)*NELherbage(:,:))      
          
        DMIf(:,:,2)=(NER(:,:,2)-DMIcowanimal(:,:,2)*NELherbage(:,:))/(7.12*EVf(:,:)-SHf(:,:)*NELherbage(:,:))                  
       ...
       ! On ne complemente pas les animaux si l'herbe suffit a couvrir les besoins energetiques
         ...
         WHERE (((DMIcowanimal(:,:,1)-Shf(:,:)*DMIf(:,:,1))*FVh(:,:)+ DMIf(:,:,1)*FVf(:,:)).gt.ICcow(:,:,1))
               DMIf(:,:,1)=(iccow(:,:,1)-(DMIcowanimal(:,:,1)-Shf(:,:)*DMIf(:,:,1))*FVh(:,:))/FVf(:,:)    
         ENDWHERE 
         
         WHERE (((DMIcowanimal(:,:,2)-Shf(:,:)*DMIf(:,:,2))*FVh(:,:)+ DMIf(:,:,2)*FVf(:,:)).gt.ICcow(:,:,2))    
               DMIf(:,:,2)=(iccow(:,:,2)-(DMIcowanimal(:,:,2)-Shf(:,:)*DMIf(:,:,2))*FVh(:,:))/FVf(:,:)
         ENDWHERE
  ...
  SUBROUTINE Milk_Animal_cow(         &
    ...
    INTEGER, INTENT(in)                            :: npts                    ! Number of spatial points (-)
    REAL(r_std ), INTENT(in)                       :: dt                      ! Time step (d)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(in)    :: nanimaltot              ! Stocking density (animal m-2)
    INTEGER(i_std ),                    INTENT(in)    :: tjulian                 ! Julian day (d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)    :: NEBcow                  ! Net energy Balance (young :1 , adult:2) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcow                   ! Daily milk production per m2 for primiparous or multiparous cows (kg/m2/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPwcow2                 ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowC                  ! C in daily milk production per m2 for primiparous or multiparous cows (kgC/m2/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowN                  ! N in daily milk production per m2 for primiparous or multiparous cows (kgN/m2/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowCsum               ! Cumulated C in milk production per m2 for primiparous or multiparous cows (kgC/m2)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowNsum               ! Cumulated N in milk production per m2 for primiparous or multiparous cows (kgN/m2)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(inout) :: milkanimalsum           ! Milk product per animal per years (L.(animal.years)-1)    
    REAL(r_std ), DIMENSION(npts,nvm)                  :: milkKG                  ! Daily actual milk production per animal for the whole cattle (kg/animal/d)

    !20/03/2009 AIG & MG
    REAL(r_std ), DIMENSION(npts,nvm)                  :: nWeeklact               ! Lactation week (in weeks from calving)
    REAL(r_std ), DIMENSION(npts,nvm,2)                :: MPwcow2max              ! Daily potential milk production per animal for primiparous or multiparous cows at peak of lactation(kg/animal/d)      
    REAL(r_std ), DIMENSION(npts,nvm)                  :: milkanimal_write        
    REAL(r_std ), DIMENSION(npts,nvm)                  :: dsevrage                ! Julian day of the suckling calf period
      ...
      DO i=1,npts
        ! Week of lactation for cows
            ...
            ! Maximum potential of lactation of a cow
                IF ((nWeeklact(i,j) .LE.43).AND.((tjulian.LT.dsevrage(i,j)).OR.(tjulian.GT.tcalving(i,j)))) THEN       
                    MPwcow2(i,j,1) = MPwcow2max(i,j,1) * ( 0.885 * nWeeklact(i,j)**(0.2) * EXP((-0.04) * nWeeklact(i,j)) )
                    MPwcow2(i,j,2) = MPwcow2max(i,j,2) * ( 0.885 * nWeeklact(i,j)**(0.2) * EXP((-0.04) * nWeeklact(i,j)) )
                ELSE
                    MPcow2(i,j,1) = 0.0    
                    MPcow2(i,j,2) = 0.0
                ENDIF    
  ...
  INTEGER, INTENT(in)                         :: npts                    ! Number of spatial points (-)
  REAL(r_std ), INTENT(in)                    :: dt                      ! Time step (d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: DMIcowanimal            ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
  INTEGER,                       INTENT(in)   :: Agecow                  ! 0:young, 1:adult
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout):: BCS                     ! Body Condition Score (for cow only /5)
  INTEGER(i_std ), INTENT(in)                    :: tjulian                 ! Julian day (-) 
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout):: wanimalcow              ! Animal liveweight (kg/animal) (young:1, adult:2)  
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: nanimaltot 	         ! Stocking rate (animal m-2)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEB                     ! Net energy balance(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: EVf                     ! Energy of the forage based (MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: Forage_quantity_period  ! Forage quantity  (MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: EVc                     ! Energy of the concentrate (MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: Qic                     ! Concentrate quantity per kg of milk or per kg of LW (MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEI                     ! Net energy intake from ingested herbage(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEIh                    ! Net energy intake from ingested herbage(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEIf                    ! Net energy intake from ingested forage(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEIc                    ! Net energy intake from ingested concentrate(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEPgest                 ! Net energy for gestation (suckler cows)(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEPlact                 ! Net energy for milk production(MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEP                     ! Net energy for production (MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEM                     ! Net energy for maintenance (MJ)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NER                     ! Total net energy requirements (maintenance and production)(MJ)


  
  !Local variable
  REAL(r_std ), DIMENSION(npts,nvm)               :: NEBcow_calc             ! tempory variable to Gain or Loss computation


  INTEGER                                     :: jourdepuisvelage        ! Calving date (-)
  INTEGER                                     :: i,j                       ! for loop
  REAL(r_std )                                :: alpha                   !parametre for NEM computation
  REAL(r_std )                                :: beta = 0.2              !parametre for NEM computation
  REAL(r_std )                                :: gamma                   !parametre for NEM computation
  REAL(r_std )                                :: delta                   !parametre for NEM computation
  ...
  INTEGER, INTENT(in)                         :: npts                    ! Number of spatial points (-)
  REAL(r_std ), INTENT(in)                    :: dt                      ! Time step (d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: DMIcowcalf              ! Calf dry matter intake (Kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: nanimaltot              ! Stocking density (animal m-2)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout):: wanimalcalf             ! Calf liveweigth (kg/animal)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)   :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEIherbage              ! Net energy intake from ingested herbage (MJ/Kg)  
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEImilk                 ! Net Erengy of ngested milk(MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEI                     ! Net energy of global intake(MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEM                     ! Net energy  metabolic(MJ/Kg)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)  :: NEG                     ! Net energy growth(MJ/Kg)
  
  !Local variable
  REAL(r_std )                                :: beta=0.2                ! Parameter for NEM computation
  REAL(r_std ), DIMENSION(npts,nvm)               :: NEG_calc                ! For compute gain weigth
  INTEGER                                     :: i,j                       ! for loop
  ...
  INTEGER, INTENT(in)                               :: npts                    ! Number of spatial points (-)
  INTEGER, INTENT(in)                               :: npta                    ! 1 : primiparous cows 2 : multiparous cows
  REAL(r_std ), INTENT(in)                          :: dt                      ! Time step (d)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: MPpos                   ! Possible milk production of dairy cows according to the diet (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2                 ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(inout) :: BCS                     ! Body Condition Score (for cow only /5)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(inout) :: BCScow_prev             ! Body Condition Score at previsou time step (for cow only /5)
  REAL(r_std ), DIMENSION(npts,nvm,npta),INTENT(in)     :: AGE_animal              ! Animal age in case of simulation of dairy cows (months)
  REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(inout) :: wanimalcow              ! Animal liveweight (kg/animal) (young:1, adult:2)  
  REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: nanimaltot              ! Stocking density (animal/m2)

    
  !Local variable
  REAL(r_std ), DIMENSION(npts,nvm,npta)               :: NEBcow_W                 ! Daily variation of cow liveweight (kg/d)
  REAL(r_std ), DIMENSION(npts,nvm,npta)               :: NEBcow_BCS               ! Daily variation of cow body condition score (/d)
      ...
      ! Primiparous cows   
         WHERE((MPwcow2(:,:,1)-MPpos(:,:,1)).LT.0) ! Liveweight and body condition increase
             NEBcow_BCS(:,:,1)=(0.44/180)*(MPpos(:,:,1)-MPcow2(:,:,1))
             NEBcow_W(:,:,1)=(0.44/3.5)*(MPpos(:,:,1)-MPcow2(:,:,1))
         ELSEWHERE                             ! Liveweight and body condition decrease
             NEBcow_BCS(:,:,1)=(0.44/240)*(MPpos(:,:,1)-MPcow2(:,:,1))
             NEBcow_W(:,:,1)=(0.44/4.5)*(MPpos(:,:,1)-MPcow2(:,:,1))                               
         ENDWHERE
       ! Multiparous cows  
         WHERE((MPwcow2(:,:,2)-MPpos(:,:,2)).LT.0) ! Liveweight and body condition increase
  ...
  SUBROUTINE balance_energy_heifer(&
    ...
    INTEGER, INTENT(in)                               :: type_animal             ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    REAL(r_std ), INTENT(in)                          :: dt                      ! Time step (d)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: nanimaltot              ! StockRate of cattle
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: DMIheifer               ! Dry Matter intake of a cow/calf (Kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: EVf                     ! Energy of the forage based (MJ/Kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: Forage_quantity_period  ! Forage quantity (MJ/Kg)
    
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(inout)      :: wanimalcow              ! Animal liveweight (kg/animal) (young:1, adult:2)  
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)        :: NEI                     ! Energy of the forage based on SEBIEN model(MJ/Kg)                        
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)        :: NEIh                    ! Net Energy intake from ingested herbage(MJ)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)        :: NEIf                    ! Net Energy intake from ingested forage(MJ)

    REAL(r_std ), DIMENSION(npts,nvm)                     :: NEIheifer_W             ! temporary variable to Gain or Loss computation
    ! These parameters come from INRA tables 2007p. + J. Agabriel UMR URH Theix
    REAL(r_std ), DIMENSION(npts,nvm)                     :: alpha                   ! Coefficient for linear regression : NEI[UFL]/LW[kg]^0.75=alpha * LWG[kg/d]^1.4 + beta
    REAL(r_std ), DIMENSION(npts,nvm)                     :: beta                    ! Coefficient for linear regression : NEI[UFL]/LW[kg]^0.75=alpha * LWG[kg/d]^1.4 + beta
    REAL(r_std ), DIMENSION(npts,nvm)                     :: denominateur            ! intermediary variable 
  INTEGER                                     :: j
  ...
  SUBROUTINE Respiration_Methane_cow(&
    ...
    ! Declarations:
    INTEGER, INTENT(in)                        :: npts          ! Number of spatial points (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: grazingc      ! C flux associated to grazing (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot    ! Stocking density (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: DNDFI         ! Amount of digestible neutral detergent fiber in the intake (kg d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: Wanimal       ! Animal life weight (kg)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: R_cow         ! Animal respiration (kg C / m²)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: CH4_cow       ! Enteric methane emission (Kg C / m²)
 ...
 SUBROUTINE Respiration_Methane_cow_2(npts, npta, type_animal, OMD,NEIh,NEIf,NEIc,grazingc,nanimaltot,&
    ...
    INTEGER, INTENT(in)                              :: npts               ! Number of spatial points (-)
    INTEGER, INTENT(in)                              :: npta               !  equals 2 when cow (young/primipare and mature/multipare) and 1 when calf
    INTEGER, INTENT(in)                              :: type_animal        ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    ...
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)        :: grazingc           ! C flux associated to grazing (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)        :: nanimaltot         ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)        :: panimaltot         ! proportion of primipare
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)       :: R_cow              ! Daily animal respiration (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out)       :: CH4                ! Daily enteric methane production (kg C/m2/d); 
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)  :: CH4animal          ! Daily enteric methane production for young or mature cows (kg C/m2/d); 
    REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)   :: MPcow2             ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(inout):: forage_complementc ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
    INTEGER, INTENT(in)                              :: f_complementation  ! Flag to activate cow complementation
                                                                        
    
    
    REAL(r_std ), DIMENSION(npts,nvm)                    :: dE          ! Energy digestibility (%)
    REAL(r_std ), DIMENSION(npts,nvm)                    :: Ymh         ! CH4 conversion factor, per cent of  metabolizable energy in ingested herbage 
    REAL(r_std ), DIMENSION(npts,nvm,npta)               :: Ymfc        ! CH4 conversion factor, per cent of  metabolizable energy in ingested forage+concentrate
    REAL(r_std ), DIMENSION(npts,nvm,npta)               :: CH4h        ! Daily enteric methane production from ingested herbage  (kg C animal-1 d-1)
    REAL(r_std ), DIMENSION(npts,nvm,npta)               :: CH4fc       ! Daily enteric methane production from ingested forage and concentrate (kg C animal-1 d-1)                                                    
       ...
       DO i=1,npts
          DO k=1,npta
            ...
            ! Methane from ingested forage and concentrate(kg C/m2/d)           
               CH4fc(i,j,k)=((8.25+0.07*(NEIf(i,j,k)+NEIc(i,j,k))/k_CH4)/55.65)*ch4toc*nanimaltot(i,j)            
             ELSE    
               CH4fc(i,j,k)=(Ymfc(i,j,k)*(NEIf(i,j,k)+NEIc(i,j,k))/(5565*k_CH4))*ch4toc*nanimaltot(i,j)            
             ENDIF
          ENDDO
       ENDDO
     ENDDO   
    ELSE  !!! for suckler cows or heifers !!!
       ...
       ! Methane from ingested forage and concentrate(kg C/m2/d)
       CH4fc(:,:,1)=Ymfc(:,:,1)*(NEIf(:,:,1)+NEIc(:,:,1))/(5565*k_CH4)*ch4toc*nanimaltot
       CH4fc(:,:,2)=Ymfc(:,:,2)*(NEIf(:,:,2)+NEIc(:,:,2))/(5565*k_CH4)*ch4toc*nanimaltot
    ENDIF   
  ...
  ! Total methane (kg C/m2/d)
    ...
    CH4(:,:)=(CH4h(:,:,1)+CH4fc(:,:,1))*panimaltot+(CH4h(:,:,2)+CH4fc(:,:,2))*(1-panimaltot)
 ...
 SUBROUTINE Urine_Faeces_cow(&
    ...
    INTEGER, INTENT(in)                        :: npts               ! Number of spatial points (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: grazingn           ! N flux associated to grazing (kg N m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: grazingc           ! C flux associated to grazing (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: forage_complementc ! C flux associated to forage anc complementation (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: forage_complementn ! N flux associated to forage anc complementation (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot         ! Stocking rate (animal m-2)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: urinen             ! urine N flux (kg N m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: faecesn            ! faeces N lux (kg N m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: urinec             ! urine C flux (kg C m-2 d-1)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(out) :: faecesc            ! faeces C flux (kg C m-2 d-1)

   !implicit variable intent(in) :
   !- fnurine : Fraction of N in excreta not volatilised, that is in urineN (Menzi et al 1997) (-) 

    ! Local variables
    REAL(r_std ), DIMENSION(npts,nvm) :: excretan               ! Total N excreta (kg N m-2 d-1)
  ...
  SUBROUTINE histwrite_cow_Part1(npts,DMIyoung,DMImature,DMicalf,pyoung_in,OMD,MPcow2,NEBcow, NEIcow, nanimaltot,type_animal,&
    ...
    INTEGER, INTENT(in)                             :: npts                 ! Number of spatial points (-)
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)       :: DMIyoung             ! Ingested dry matter for calf (Kg/d)         
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)       :: DMImature            ! Ingested dry matter for calf (Kg/d)         
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)       :: DMIcalf              ! Daily calf intake per m2 (Kg/d)          
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)       :: pyoung_in            ! Ingested dry matter for calf (Kg/d)         
    REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)       :: OMD                  ! Digestible organic matter in the intake(kg/kg)
    
    REAL(r_std ), DIMENSION(npts,nvm)                   :: BCScows              ! Average BCS of cattle
    REAL(r_std ), DIMENSION(npts,nvm)                   :: Weightcows           ! Average weight of cattle 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: MPcow2               ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: NEBcow               ! Net energy Balance (young :1 , adult:2) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: NEIcow               ! Net energy intake (MJ)
    REAL(r_std ), DIMENSION(npts,nvm)                   :: nanimaltot           ! Stocking density (animal/m2)
    INTEGER, INTENT(in)                             :: type_animal          ! 1 or 2 or 4 or 5= > new module animal
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: MPwcow2              ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: MPpos                ! Possible milk production of dairy cows according to the diet (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: DMIc                    ! Concentrate intake (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: DMIf                    ! forage intake (kg/animal/d)
...
INTEGER function Verif_management(npts,nstocking,tanimal,danimal)
    INTEGER, INTENT(in)                                    :: npts         ! Number of spatial points (-)
    INTEGER, INTENT(in)                                    :: nstocking         ! Number of spatial points (-)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(in)    :: tanimal      ! Beginning of the grazing period    h (1,..,nstocking) (d)
    REAL(r_std ), DIMENSION(npts,nvm,nstocking), INTENT(in)    :: danimal      ! Lenght of the grazing period    h (1,..,nstocking) (d)        
    !Local
    ...
    !On regarde si il y a cumule de periode, si oui STOP RUN       
        ...
        h  = 1
            ...
            DO WHILE(h .LT. nstocking)
            WHERE((J .GE. tanimal(:,:,h)) .AND. (J .LT. (tanimal(:,:,h) + danimal(:,:,h))))
...
SUBROUTINE calcul_perte_etat(npts,tjulian,BCScow,MPwmax,tcalving,PEmax) 
  ...
  INTEGER, INTENT(in)                                    :: npts         ! Number of spatial points (-)
  INTEGER(i_std ), INTENT(in)                               :: tjulian      ! Julian day
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)            :: BCScow       ! Body Condition Score (for cow only /5)
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)            :: MPwmax       ! Maximum of theoretical milk production (kg/animal/d)
  REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)              :: tcalving     ! Calving date (d) 
  REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)           :: PEmax        ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
 ...
 ! si tjulian est inférieur à tcalving on considere le velage de l'annee precedente   
  ...
  ! On borne dans ce cas la perte d'etat max a zero car celle ci doit être signee negativement
     ...
     PEmax(:,:,1)=0.52615+7*0.0042*nWeekLact(:,:)-0.01416*MPwmax(:,:,1)-0.3644*BCScow(:,:,1)
     PEmax(:,:,2)=0.66185+7*0.0042*nWeekLact(:,:)-0.01416*MPwmax(:,:,2)-0.3644*BCScow(:,:,2)
...
!----------------------------------------
  ...
  SUBROUTINE Calcul_NER_cow(npts,npta,wanimalcow,wcalfborn, Age_animal, nweekgest, MPwcow2,NER,NEGcow,NEMcow)
      ...
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: wanimalcow              ! Animal liveweight (kg/animal) (young:1, adult:2) 
      REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: Wcalfborn               ! Calf liveweigth at birth (kg/animal)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: AGE_animal              ! Animal age in case of simulation of dairy cows (months)
      REAL(r_std ), DIMENSION(npts,nvm),      INTENT(in)    :: Nweekgest               ! Gestation week (in weeks from mating)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2                 ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NER                     ! Total net energy required (MJ)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEGcow                  ! Net energy required for gestation (MJ)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEMcow                  ! Net energy required for gestation (MJ)
      REAL(r_std ), DIMENSION(npts,nvm,npta)                :: NEPlact                 ! Net energy required for milk prduction (MJ)
         ...
         WHERE (nweekgest.LE.40)
          NEGcow(:,:,1)=7.12*(3.25-0.08*Age_animal(:,:,1) + 0.00072*wcalfborn(:,:)*exp(0.116*nweekgest(:,:)))
          NEGcow(:,:,2)=7.12*(3.25-0.08*Age_animal(:,:,2) + 0.00072*wcalfborn(:,:)*exp(0.116*nweekgest(:,:)))
         ENDWHERE
  ...
  SUBROUTINE calcul_NEI_cow_d(npts,npta,MPcow2,DMIcowanimal,NELherbage,&
      ...
      INTEGER, INTENT(in)                               :: npts                    ! Number of spatial points (-)
      INTEGER, INTENT(in)                               :: npta                    ! 
      REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)       :: MPcow2                  ! Daily actual milk production per animal for primiparous or multiparous cows at previous time step (kg/animal/d)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: DMIcowanimal            ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
      REAL(r_std ), DIMENSION(npts,nvm)     , INTENT(in)    :: NELherbage              ! Energetic content of the herbage (MJ/kg) 
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: EVf                     ! Energy of the forage based (MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: Forage_quantity_period  ! Forage quantity  (MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm), INTENT(in)         :: EVc                     ! Energy of the concentrate (MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(in)    :: Qic                     ! Concentrate quantity per kg of milk or per kg of LW (MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEI                     ! Net energy intake(MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEM                     ! Net energy intake(MJ/Kg)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIh                    ! Net Energy intake from ingested herbage(MJ)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIf                    ! Net Energy intake from ingested forage(MJ)
      REAL(r_std ), DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIc                    ! Net Energy intake from ingested concentrate(MJ)
  ...
  SUBROUTINE Potentiel_dairy_d(npts,tjulian,nweekLact,nweekGest,MPwcow2max,MPwcow2)
    ...
    INTEGER, INTENT(in)                             :: npts                    ! Number of spatial points (-)
    INTEGER(i_std ),                    INTENT(in)     :: tjulian                 ! Julian day (d)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)    :: nWeeklact               ! Lactation week (in weeks from calving)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)    :: nWeekGest               ! Gestation week (in weeks from mating)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)     :: MPwcow2max              ! Daily potential milk production per animal for primiparous or multiparous cows at peak of lactation(kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)    :: MPwcow2                 ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    ...
    !------------------------------
        ...
        MPwcow2(:,:,1)=MPwcow2max(:,:,1)*(1.084-(0.7*exp(-0.46*nWeeklact(:,:)))-(0.009*nWeeklact(:,:))-(0.69*exp(-0.16*(45-nweekgest(:,:)))))   
        MPwcow2(:,:,2)=MPwcow2max(:,:,2)*(1.047-(0.69*exp(-0.90*nWeeklact(:,:)))-(0.0127*nWeeklact(:,:))-(0.5*exp(-0.12*(45-nweekgest(:,:)))))   
  ...
  SUBROUTINE Milk_Animal_cow_d(                &
    ...
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)     :: MPcowCsum               ! Cumulated C in milk production per m2 for primiparous or multiparous cows (kgC/m2)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)     :: MPcowNsum               ! Cumulated N in milk production per m2 for primiparous or multiparous cows (kgN/m2)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(inout)   :: milkanimalsum           ! Milk production per animal and per year (L.(animal.year)-1)    
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)     :: nWeeklact               ! Lactation week (in weeks from calving)
    REAL(r_std ), DIMENSION(npts,nvm)  , INTENT(out)     :: nWeekGest               ! Gestation week (in weeks from mating)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)      :: PEmax                   ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)   :: PEpos                   ! Perte d'etat possible des vaches laitières au jour j
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)      :: deltaBCS                ! Body condition score variation between two consecutive time steps (-)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)     :: MPpos                   ! Possible milk production of dairy cows according to the diet (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)      :: NEIcow                  ! Total net energy intake (1:young, 2:adult) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)      :: NEMcow                  ! Net energy for maintenance (young :1 , adult:2) (MJ)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(in)      :: NEGcow                  ! Net energy for gestation (dairy cows)(young :1 , adult:2) (MJ) 
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(inout)   :: MPcow2_prec             ! Daily actual milk production per animal for primiparous or multiparous cows at previous time step (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2), INTENT(out)     :: MPpot                   ! Potential milk production (kg/d)

    REAL(r_std ), DIMENSION(npts,nvm)                   :: milkKG                  ! Daily actual milk production per animal for the whole cattle (kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm,2)                 :: MR                      ! Milk response (-)
    REAL(r_std ), DIMENSION(npts,nvm,2)                 :: RF                      ! Remobilisation fraction (-)
    REAL(r_std ), DIMENSION(npts,nvm)                   :: Fremob                  ! facteur de remobilisation (fonction de la lactation)
    REAL(r_std ), DIMENSION(npts,nvm,2)                 :: MPwcow2max              ! Daily potential milk production per animal for primiparous or multiparous cows at peak of lactation(kg/animal/d)
    REAL(r_std ), DIMENSION(npts,nvm)                   :: milkanimal_write        ! Milk production per animal and per day (kg animal-1 d-1)
    REAL(r_std ), DIMENSION(npts,nvm,2)                 :: min_NEB                 ! minimum value of NEB for milk production calculation
    INTEGER                                         :: i,k,j                       ! For loop
    
    MPwcow2max(:,:,1)=MPwmax(:,:,1) ! potential milk production of primiparous cows (kg) 
    MPwcow2max(:,:,2)=MPwmax(:,:,2) ! potential milk production of multiparous cows (kg)
    ...
    MPpos(:,:,2)=max(0.0,(NEIcow(:,:,2)-NEMcow(:,:,2)-NEGcow(:,:,2))/(0.44*7.12))
           ...
           !------------------------------
            ...
            !----------------------------------------------------
            MPpot(:,:,1)=MPwcow2max(:,:,1)*(1.084-(0.7*exp(-0.46*nWeeklact))-(0.009*nWeeklact)-(0.69*exp(-0.16*(45-nweekgest))))   
            MPpot(:,:,2)=MPwcow2max(:,:,2)*(1.047-(0.69*exp(-0.90*nWeeklact))-(0.0127*nWeeklact)-(0.5*exp(-0.12*(45-nweekgest))))   
            ...
            !-----------------------------------------------------------
                WHERE(nWeeklact .LE.43)                                        
                    ...
                    ! AIG June 2010 to avoid that milk production could be negative
                       ...
                       MPcow2(:,:,1)=max(0.0,MPpos(:,:,1)-MR(:,:,1)*(MPpos(:,:,1)-MPpot(:,:,1)))
                    ELSEWHERE
                       MPcow2(:,:,1)=MPpot(:,:,1)   
                    ENDWHERE                 
                    ...
                    ! AIG June 2010 to avoid that milk production could be negative
                       ...
                       MPcow2(:,:,2)=max(0.0,MPpos(:,:,2)-MR(:,:,2)*(MPpos(:,:,2)-MPpot(:,:,2)))
                    ELSEWHERE
                       MPcow2(:,:,2)=MPpot(:,:,2)   
                    ENDWHERE                                                
