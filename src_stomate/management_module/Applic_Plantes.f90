...
CONTAINS
  ...
  SUBROUTINE Main_appl_pre_animal(&
    ...
    initialisation : IF (first_call) THEN 
      first_call = .FALSE. 

        ALLOCATE (tamean1        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tamean2        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tamean3        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tamean4        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tamean5        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tamean6        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tameand        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tameanw        (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tacumm         (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tacummprev     (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tsoilcumm      (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tsoilcummprev  (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tsoilmeand     (npts), stat=ier); l_error=l_error .OR. (ier .NE. 0)
        ALLOCATE (tcut0          (npts,nvm), stat=ier); l_error=l_error .OR. (ier .NE. 0)
    ...
    n_day : IF (new_day) THEN
      ...
      DO j=2,nvm
        DO i=1,npts
          ...
          IF ((devstage(i,j) .LE. 0.0) .AND. ( (tameanw(i) .GT. trep) .OR. (regcount(i,j) .EQ. 2) ) ) THEN
              
              devstage(i,j) = MAX(0.0, tameand(i) - tbase)/tasumrep 
              
          ELSEIF ((devstage(i,j) .GT. 0.0) .AND. (tsoilmeand(i) .GT. tbase) .AND. (devstage(i,j) .LT. 2.0) ) THEN
