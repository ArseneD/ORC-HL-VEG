  ...
  SUBROUTINE cutting_spa(&
    ...
    DO j=2,nvm
      WHERE(flag_cutting(:,j) .EQ. 1) 
        ...
        wlam(:,j) = ( biomass(:,j,ileaf,icarbon)/(1000*CtoDM) )/(1.0 + (mc /12.0)*c(:,j) + (mn /14.0)*n(:,j) )
        wst(:,j)  = ( biomass(:,j,isapabove,icarbon)/(1000*CtoDM) )/(1.0 + (mc /12.0)*c(:,j) + (mn /14.0)*n(:,j) )
        wear(:,j) = ( biomass(:,j,ifruit,icarbon)/(1000*CtoDM) )/(1.0 + (mc /12.0)*c(:,j) + (mn /14.0)*n(:,j) )
      END WHERE
...
! aera simulated
      ...
      DO i=1,npts
        IF (flag_cutting(i,j) .EQ. 1) THEN
          wshcutinit(i,j) = MIN (wsh(i,j), wshtotcutinit(i,j,regcount(i,j) + 1)/(1.0 + (mc/12.0)*c(i,j) + (mn/14.0)*n(i,j)))
        END IF
      END DO
      WHERE (flag_cutting(:,j) .EQ. 1) 
