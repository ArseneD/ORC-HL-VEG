...
!!
!! SVN :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_stomate/stomate.f90 $
...
!$OMP THREADPRIVATE(veget_cov_max_new)
...
!$OMP THREADPRIVATE(PFTpresent)
  ...
  LOGICAL,ALLOCATABLE,SAVE,DIMENSION(:,:)        :: begin_leaves         !! onset
...
!$OMP THREADPRIVATE(t2m_daily)
  ! DZ modify: "seasonal" 2 meter temperatures (K)
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)      :: Tseason
  ! temporary variable to calculate Tseason
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)      :: Tseason_length
  ! temporary variable to calculate Tseason
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)      :: Tseason_tmp

  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: Tmin_spring
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:)      :: Tmin_spring_time
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:,:,:)      :: onset_date
...
!$OMP THREADPRIVATE(lignin_struc)
  REAL(r_std),ALLOCATABLE,SAVE,DIMENSION(:)      :: black_carbon         !! Black carbon on the ground 
                                                                         !! @tex $(gC m^{-2})$ @endtex   
!$OMP THREADPRIVATE(black_carbon)
!---
...
!$OMP THREADPRIVATE(do_daily_permafrost)
...
!$OMP THREADPRIVATE(ok_equilibrium)
  LOGICAL, ALLOCATABLE, SAVE, DIMENSION(:)           :: ok_equilibrium_g    !! Logical array marking the points where the resolution is ok 
                                                                            !! (for parallelisation) (true/false)
!$OMP THREADPRIVATE(ok_equilibrium_g)
...
!$OMP THREADPRIVATE(nparan)
    INTEGER(i_std),SAVE                           :: nbyear=1                 !! Number of years saved for forcesoil - read from 
...
!ENDJCADD
       ...
       IF (ldcarbon_write .AND. TRIM(Cforcing_permafrost_name) /= 'NONE' ) THEN
          WRITE(numout,*) &
...
!! 3. Special treatment for some input arrays.
    ...
    IF ( long_print ) WRITE(*,*) 'cdk debug stomate: prep to calc fbact'
  ...
  !! 4. Calculate variables for dt_slow (i.e. "daily")
    ...
    IF ( do_daily_permafrost ) THEN
       DO iv = 1,nvm
          CALL stomate_accu(kjpindex, ndeep, dt_slow, dtradia, &
       ...
       !adding two forcings
       CALL stomate_accu(kjpindex, nsnow, dt_slow, dtradia, &
...
! JCADD daily resp_hetero_litter
!    resp_hetero_litter_d(:,:) = resp_hetero_litter_d(:,:) + resp_hetero_litter(:,:)
!    WRITE (numout,*) 'litter resp daily',resp_hetero_litter_d(:,:)
!    WRITE (numout,*) 'litter resp',resp_hetero_litter(:,:)
! ENDJCADD
    ...
    IF ( ok_pc ) THEN
       IF ( .NOT. do_daily_permafrost ) THEN
          ...
          CALL deep_carbcycle(kjpindex, index, itime, dtradia, lalo, clay, &
       ...
       ELSE
          ...
          DO iv = 1,nvm
             CALL stomate_accu(kjpindex, ncarb, dt_slow, dtradia, &
...
!    resp_hetero_soil_part(:,:,:)=resp_hetero_soil_part(:,:,:)*dtradia/one_day
    ...
    IF (do_slow) THEN
       ...
       IF ( ok_pc ) THEN
          IF ( do_daily_permafrost ) THEN
             ...
             CALL deep_carbcycle(kjpindex, index, itime, dt_slow, lalo, clay, &
       ...
       IF(CH4_calcul) THEN
          ...
          IF (CH4_WTD1) THEN
             ...
             CALL ch4_wet_flux_density_wet1 (kjpindex,dtradia,stempdiag,tsurf_daily,tsurf_year,veget_cov_max,veget,&
          ...
          IF (CH4_WTD2) THEN
             ...
             CALL ch4_wet_flux_density_wet2 (kjpindex,dtradia,stempdiag,tsurf_daily,tsurf_year,veget_cov_max,veget,&
          ...
          IF (CH4_WTD3) THEN
             ...
             CALL ch4_wet_flux_density_wet3 (kjpindex,dtradia,stempdiag,tsurf_daily,tsurf_year,veget_cov_max,veget,&
          ...
          IF (CH4_WTD4) THEN
             ...
             CALL ch4_wet_flux_density_wet4 (kjpindex,dtradia,stempdiag,tsurf_daily,tsurf_year,veget_cov_max,veget,&
...
!JCADD
               ...
               &             N_limfert)!,resp_hetero_litter_d,resp_hetero_soil_d)
!ENDJCADD
...
!!$             CALL histwrite_p (hist_id_stomate, 'CO2FLUX_MONTHLY_SUM', itime, &
!!$                  vartmp, kjpindex, hori_index )
!!$          ENDIF
!!$          IF (is_root_prc) THEN
!!$             OPEN( unit=39,              &
...
!!$          ENDIF
       ...
       IF (spinup_analytic) THEN
          nbp_accu(:) = nbp_accu(:) + (-SUM(co2_flux_daily(:,:) * veget_max(:,:),dim=2) - (convflux(:) + cflux_prod10(:) + &
    ...
    IF (spinup_analytic) THEN
       ...
       IF (EndOfYear) THEN
          ...
          IF( MOD(global_years, spinup_period) == 0 ) THEN
             ...
             IF(is_root_prc) THEN         
                IF(ALL(ok_equilibrium_g)) THEN
                   WRITE(numout,*) 'Spinup analytic : Equilibrium for carbon pools is reached'
                ELSE
                   WRITE(numout,*) 'Spinup analytic : Equilibrium for carbon pools is not yet reached'
                END IF
             ENDIF
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER(i_std)                               :: ji,j,ipd,l,iv     !! Indices
...
!_ ================================================================================================================================
  ...
  !! 1. Online diagnostics
    ...
    !Config Units = [-]
    !
    ! By default, ::bavard is set to 1 in stomate_data
    bavard = 1
  ...
  !! 4. Allocate memory for STOMATE's variables
    ...
    ALLOCATE(Tmin_spring(kjpindex,nvm),stat=ier)
    l_error = l_error .OR. (ier /= 0)
    IF (l_error) THEN
       WRITE(numout,*) 'Memory allocation error for Tmin_spring. We stop. We need kjpindex*nvm words',kjpindex,nvm
       STOP 'stomate_init'
    ENDIF
...
!_ ================================================================================================================================
  ...
  !! 1. Deallocate all dynamics variables
    ...
    IF (ALLOCATED(Tmin_spring)) DEALLOCATE(Tmin_spring)
...
!ENDJCADD
    ...
    IF (is_root_prc) THEN
       ...
       IF (ALLOCATED(ok_equilibrium_g)) DEALLOCATE(ok_equilibrium_g)
    ENDIF
