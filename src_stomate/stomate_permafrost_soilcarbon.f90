...
!================================================================================================================================
  ...
  SUBROUTINE deep_carbcycle(kjpindex, index, itau, dtradia, lalo, clay, &
       ...
       rprof, altmax, carbon, carbon_surf, resp_hetero_soil, dt_sechiba, fbact, fixed_cryoturbation_depth, &
       snowdz,snowrho)
...
!! 0. Variable and parameter declaration    
    ...
    REAL(r_std), INTENT(in)                               :: dt_sechiba        !! timestep of overall model, for calendar calcs
    ...
    IF (bavard .GE. 10) WRITE(*,*) 'Entering  deep_carbcycle'
    ...
    IF ( firstcall ) THEN                
       ...
       !
       CALL soil_gasdiff_main (kjpindex,dtradia,index,'initialize', &
       ...
       !
       CALL soil_gasdiff_main (kjpindex,dtradia,index,'coefficients', &
            pb,tsurf,tprof,diffO2_snow,diffCH4_snow, &
            totporO2_snow,totporCH4_snow,O2_snow,CH4_snow,diffO2_soil,diffCH4_soil, &
            totporO2_soil,totporCH4_soil,O2_soil,CH4_soil, zi_snow, zf_snow)
       ...
       CALL altcalc (kjpindex, dtradia, dayno, scnd, REAL(tprof), zi_soil, alt, alt_ind, altmax, altmax_ind, &
    ...
    IF ( .NOT. no_pfrost_decomp ) THEN
            ...
            !
            CALL soil_gasdiff_main (kjpindex, dtradia,index, 'diffuse', &
                 pb,tsurf,tprof,diffO2_snow,diffCH4_snow, &
                 totporO2_snow,totporCH4_snow,O2_snow,CH4_snow,diffO2_soil,diffCH4_soil, &
                 totporO2_soil,totporCH4_soil,O2_soil,CH4_soil, zi_snow, zf_snow)
            ...
            IF (ok_cryoturb) CALL cryoturbate(kjpindex, dtradia, dayno, altmax_ind_lastyear, deepC_a, deepC_s, deepC_p, &
                 'diffuse', cryoturbation_diff_k_in/(one_day*one_year), &
                 altmax_lastyear, fixed_cryoturbation_depth)
            !
            ! Carbon input into the soil
            !
             CALL carbinput(kjpindex,dtradia,itau*dtradia,no_pfrost_decomp,tprof,tsurf,hslong,dayno,z_root,altmax_lastyear, &
                  deepC_a, deepC_s, deepC_p, soilc_in, dc_litter_z, z_organic, veget_max_bg, rprof)
             !
  
             CALL permafrost_decomp (kjpindex, dtradia, tprof, frozen_respiration_func, airvol_soil, &
             ...
             DO ip = 1, kjpindex
                DO iv = 1, nvm
                   IF ( veget_mask_2d(ip,iv) ) THEN
                      ...
                      !
                      heat_Zimov(ip,:,iv) = heat_Zimov(ip,:,iv)/dtradia
             ...
             IF  ( .NOT. firstcall) THEN 
                ...
                !
               CALL traMplan(CH4_soil,O2_soil,kjpindex,dtradia,totporCH4_soil,totporO2_soil,z_root, &
               ...
               CALL ebullition (kjpindex,dtradia,tprof,totporCH4_soil,hslong,CH4_soil,febul)
            ...
            DO ip = 1, kjpindex
               ! Total CH4 flux
               sfluxCH4_deep(ip) = SUM(veget_max_bg(ip,:)*( CH4ii(ip,:)-CH4i(ip,:)+MG(ip,:)-MT(ip,:) ))/dtradia
               ! TotalCO2 flux
               sfluxCO2_deep(ip) = SUM(veget_max_bg(ip,:)*( dC1i(ip,:) + MT(ip,:)*(12./16.) ) )/dtradia  
            END DO
  
            resp_hetero_soil(:,:) = ( dC1i(:,:) + MT(:,:)*(12./16.) ) *one_day/dtradia
            sfluxCH4(:,:) = ( CH4ii(:,:)-CH4i(:,:)+MG(:,:)-MT(:,:) ) *one_day/dtradia
  
  
            ! calculate coefficients for cryoturbation calculation
            IF (ok_cryoturb) CALL cryoturbate(kjpindex, dtradia, dayno, altmax_ind_lastyear, deepC_a, deepC_s, deepC_p, &
                 'coefficients', cryoturbation_diff_k_in/(one_day*one_year), &
                 altmax_lastyear, fixed_cryoturbation_depth)
            ...
            IF (bavard .GE. 10) WRITE(*,*) 'after calc_vert_int_soil_carbon'
    ENDIF
          ...
          IF (permafrost_veg_exists(jv)) THEN  !don't bother to write if there are pfts that don't exist in our domain
             ...
             IF (writehist_deltaC_litter) THEN
                CALL histwrite_p (hist_id_stomate, 'deltaC_litter_act_'//part_str(1:LEN_TRIM(part_str)), &
                     itime, dc_litter_z(:,iactive,:,jv)/ dtradia, kjpindex*ndeep, horideep_index)
                CALL histwrite_p (hist_id_stomate, 'deltaC_litter_slo_'//part_str(1:LEN_TRIM(part_str)), &
                     itime, dc_litter_z(:,islow,:,jv)/ dtradia, kjpindex*ndeep, horideep_index)
             ENDIF
    ...
    ENDIF
    IF (bavard .GE. 10) WRITE(*,*) 'cdk: leaving  deep_carbcycle'
...
!_
!================================================================================================================================  
  SUBROUTINE altcalc (kjpindex,dtradia,dayno,scnd, temp, zprof, alt, alt_ind, altmax, altmax_ind, &
       altmax_lastyear, altmax_ind_lastyear)
  ...
  !! 0. Variable and parameter declaration
       ...
       DO ix = 1, kjpindex
          DO iv = 1, nvm
             IF ( veget_mask_2d(ix,iv) ) THEN
                DO il=1,ndeep
                   IF ( altmax(ix,iv) .GE. zi_soil(il) ) THEN
    ...
    IF (bavard .GE. 10) WRITE(*,*) 'leaving  altcalc'
...
!_
!================================================================================================================================   
  SUBROUTINE soil_gasdiff_main( kjpindex,dtradia,index,action, &
  ...
  !! 0. Variable and parameter declaration
    ...
    ELSEIF ( action .EQ. 'diffuse' ) THEN
       ...
       !
       CALL soil_gasdiff_diff( kjpindex,dtradia,index,psol,tsurf, O2_snow, CH4_snow, O2_soil, CH4_soil)
       !
    ELSEIF ( action .EQ. 'coefficients' ) THEN
       !
       ! 2.3 calculate coefficients (heat flux and apparent surface heat capacity)
       !
       CALL soil_gasdiff_coeff( kjpindex,dtradia,tprof,O2_snow,CH4_snow, &
    ...
    IF (bavard .GE. 10) WRITE(*,*) 'leaving  soil_gasdiff_main'
...
!================================================================================================================================   
  ...
  SUBROUTINE soil_gasdiff_coeff( kjpindex,dtradia,tprof,O2_snow,CH4_snow, &
       diffO2_snow,diffCH4_snow,totporO2_snow,totporCH4_snow,O2_soil,CH4_soil, &
       diffO2_soil,diffCH4_soil,totporO2_soil,totporCH4_soil, zi_snow, zf_snow)
    ...
    DO il = 1,nsnow-1
       ...
       WHERE ( snow_height_mask_2d(:,:) .AND. veget_mask_2d(:,:) )
          ...
          xcO2_snow(:,il,:) = ( zf_snow(:,il,:) - zf_snow(:,il-1,:) ) * &
               totporO2_snow(:,il,:) / dtradia
          xcCH4_snow(:,il,:) = ( zf_snow(:,il,:) - zf_snow(:,il-1,:) ) * &
               totporCH4_snow(:,il,:) / dtradia
          !
    ...
    DO il = 1,ndeep-1
       ...
       WHERE ( veget_mask_2d(:,:) )
          ...
          xcO2_soil(:,il,:) = ( zf_soil(il) - zf_soil(il-1) ) * &
               totporO2_soil(:,il,:) / dtradia
          xcCH4_soil(:,il,:) = ( zf_soil(il) - zf_soil(il-1) ) * &
               totporCH4_soil(:,il,:) / dtradia
          !
...
!================================================================================================================================    
  ...
  SUBROUTINE soil_gasdiff_diff( kjpindex,dtradia,index,pb,tsurf, O2_snow, CH4_snow, O2_soil, CH4_soil)
...
!_
!================================================================================================================================    
  SUBROUTINE traMplan(CH4,O2,kjpindex,dt,totporCH4,totporO2,z_root,rootlev,Tgr,Tref,hslong,flupmt, &
       refdep, zi_soil, tprof)
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER(i_std), INTENT(in)			                  :: kjpindex    
    REAL(r_std), INTENT(in)                                       :: dt             !! time steps in seconds 
       ...
       DO iv = 1, nvm
          IF ( (z_root(ip,iv) .GT. 0.) .AND. veget_mask_2d(ip,iv) ) THEN ! added this to prevent pmt calcs when soil frozen
             DO il=1,rootlev(ip,iv)
                ...
                ! O2 concentration is decreased in reaction with dCH4*Pox*dt 
                ...
                ! Integration	 
                flupmt(ip,iv) = flupmt(ip,iv) + dCH4(ip,iv)*totporCH4(ip,il,iv)/dt * (1 - Pox) * & 
                     ( zf_soil(il) - zf_soil(il-1) )
             ENDDO
          END IF
       ENDDO
...
!_
!================================================================================================================================     
  SUBROUTINE ebullition (kjpindex,dtradia,tprof,totporCH4_soil,hslong,Ch4_soil,febul)
                ...
                DO il = ndeep, 1, -1
                   ...
                   IF (CH4d .GT. EPSILON(0.)) THEN  
                      ...
                      febul(ip,iv) = febul(ip,iv) + dCH4 * totporCH4_soil(ip,il,iv) *  &
                           ( zf_soil(il) - zf_soil(il-1) ) / dtradia
...
!_
!================================================================================================================================     
  SUBROUTINE carbinput(kjpindex,dtradia,time,no_pfrost_decomp,tprof,tsurf,hslong, dayno,z_root,altmax, &
  ...
  !! 0. Variable and parameter declaration
    ...
    CHARACTER(LEN=16)			                             :: buf  
    INTEGER				                             :: recn
    LOGICAL, SAVE                                                    :: correct_carboninput_vertprof = .FALSE.
    ...
    ELSE	 
       ...
       soilc_in_TS(:,:,:) = soilc_in(:,:,:)*dtradia/one_day
...
!================================================================================================================================     
  ...
  SUBROUTINE cryoturbate(kjpindex, dtradia, dayno, altmax_ind, deepC_a, deepC_s, deepC_p, &
       action, diff_k_const, altmax_lastyear, fixed_cryoturbation_depth)
  ...
  !! 0. Variable and parameter declaration
    ...
    ELSEIF ( action .EQ. 'coefficients' ) THEN
       ...
       cryoturb_location(:,:) =  ( altmax_lastyear(:,:) .LT. max_cryoturb_alt ) &
            .AND. ( altmax_lastyear(:,:) .GT. 0 ) .AND. veget_mask_2d(:,:)
             ...
             IF ( cryoturb_location(ip,iv) ) THEN
                ...
                IF (use_new_cryoturbation) THEN
                   ...
                   CASE(4)
                      ...
                      END DO
                      ! 
                      IF (bavard .GE. 3) WRITE(*,*) 'cryoturb method 4: ip, iv, diff_k(ip,:,iv): ', ip, iv, diff_k(ip,:,iv)
                   CASE(5)
                      !
                      ...
                      END DO
                      !
                      IF (bavard .GE. 3) WRITE(*,*) 'cryoturb method 5: ip, iv, diff_k(ip,:,iv): ', ip, iv, diff_k(ip,:,iv)
                   END SELECT
                   
                   ELSE ! old cryturbation scheme 
                   !
       ...
       DO il = 1,ndeep-1
          WHERE ( cryoturb_location(:,:) )
             xc_cryoturb(:,il,:) = (zf_soil(il)-zf_soil(il-1))  / dtradia
             xd_cryoturb(:,il,:) = diff_k(:,il,:) / (zi_soil(il+1)-zi_soil(il))
          endwhere
       ENDDO
       
       WHERE ( cryoturb_location(:,:) )
          xc_cryoturb(:,ndeep,:) = (zf_soil(ndeep)-zf_soil(ndeep-1))  / dtradia
...
!================================================================================================================================     
  ...
  SUBROUTINE permafrost_decomp (kjpindex, dtradia, tprof, Nconfun, airvol_soil, &
       oxlim, tau_CH4troph, ok_methane, fbactratio, O2m, &
       totporO2_soil, totporCH4_soil, hslong, clay, &
       no_pfrost_decomp, deepC_a, deepC_s, deepC_p, deltaCH4g, deltaCH4, deltaC1_a, deltaC1_s, deltaC1_p, deltaC2, &
          ...
          IF (  veget_mask_2d(ip,iv) ) THEN
             ...
             DO il = 1, ndeep
                ...
                IF (oxlim) THEN
                   ...
                   dC = MIN(deepC_a(ip,il,iv) * dtradia/fbact_a,dCm)
                ELSE
                   dC = deepC_a(ip,il,iv) * dtradia/fbact_a
                ENDIF
                ...
                IF (oxlim) THEN
                   dCm = O2_soil(ip,il,iv)*airvol_soil(ip,il,iv)*wC/wO2
                   dC = MIN(deepC_s(ip,il,iv) * dtradia/fbact_s,dCm)
                ELSE
                   dC = deepC_s(ip,il,iv) * dtradia/fbact_s
                ENDIF
                ! flux vers les autres reservoirs
                ...
                IF (oxlim) THEN
                   dCm = O2_soil(ip,il,iv)*airvol_soil(ip,il,iv)*wC/wO2
                   dC = MIN(deepC_p(ip,il,iv) * dtradia/fbact_p,dCm)
                ELSE
                   dC = deepC_p(ip,il,iv) * dtradia/fbact_p
                ENDIF
                ! flux vers les autres reservoirs
                ...
                IF (ok_methane) THEN
                   ...
                   dC = deepC_a(ip,il,iv) * dtradia / fbactCH4_a * EXP(-O2_soil(ip,il,iv)*(1+hslong(ip,il,iv) * &
                   ...
                   IF ( MG_useallCpools ) THEN
                      ...
                      dC = deepC_s(ip,il,iv) * dtradia / fbactCH4_s * EXP(-O2_soil(ip,il,iv)*(1+hslong(ip,il,iv) * &
                           (BunsenO2-1.)) / O2m ) !DKtest: when commented, no ox lim for MG
                      ...
                      dC = deepC_p(ip,il,iv) * dtradia / fbactCH4_p * EXP(-O2_soil(ip,il,iv)*(1+hslong(ip,il,iv) * &
                           (BunsenO2-1.)) / O2m ) !DKtest: when commented, no ox lim for MG
                   ...
                   IF ( temp .GE. zero ) THEN
                      ...
                      dCH4 = MIN( CH4_soil(ip,il,iv) * dtradia/MAX(tau_CH4troph,dtradia), dCH4m )
                      CH4_soil(ip,il,iv) = CH4_soil(ip,il,iv) - dCH4
