...
!! SVN :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_stomate/stomate_resp.f90 $
!! $Date: 2014-02-11 15:11:28 +0100 (Tue, 11 Feb 2014) $
!! $Revision: 1889 $
!! \n
!_ ================================================================================================================================
...
!! with the units @tex $(gC.m^{-2}dt^{-1})$ @endtex, and the convention is from plants to the 
!! atmosphere.
!!
...
!! 0. Variable and parameter declaration
    ...
    REAL(r_std), INTENT(in)                            :: dt        !! Time step of the simulations (seconds)
    REAL(r_std), DIMENSION(npts), INTENT(in)           :: t2m       !! 2 meter air temperature - forcing variable (K)
    REAL(r_std), DIMENSION(npts), INTENT(in)           :: tlong_ref !! Long term annual mean 2 meter reference air temperatures 
...
!ENDJCADD
    ...
    REAL(r_std), SAVE, DIMENSION(0:nbdl)    :: z_soil               !! Variable to store depth of the different soil layers (m)
...
!_ ================================================================================================================================
 ...
 !! 3. Calculate maintenance respiration
    ...
    ! The maintenance respiration @tex $(gC.m^{-2}dt^{-1})$ @endtex of each plant compartment for each PFT is 
    ! the biomass @tex $(gC.m^{-2})$ @endtex of the plant part multiplied by maintenance respiration 
    ! coefficient @tex $(g.g^{-1}dt^{-1})$ @endtex, except that the maintenance respiration of leaves is 
...
!    IF (bavard.GE.4) WRITE(numout,*) 'Leaving maintenance respiration'	
