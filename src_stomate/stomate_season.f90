...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/trunk/ORCHIDEE/src_stomate/stomate_season.f90 $ 
...
!>\BRIEF          Flag setting 
!!
!! DESCRIPTION  : This subroutine sets flags ::firstcall, to .TRUE., and therefore activates   
...
!! RECENT CHANGE(S):None
!!
!! MAIN OUTPUT VARIABLE(S): ::firstcall
!!
!! REFERENCE(S)  : None 
!!
!! FLOWCHART     : None
...
!ENDJCADD
    ...
    REAL(r_std), DIMENSION(npts), INTENT(inout)            :: tlong_ref                 !! "long term" reference 2-meter 
                                                                                        !! temperatures (K) 
    ...
    ! DZ modify:"seasonal" 2-meter temperatures (K), to constrain boreal treeline
    REAL(r_std), DIMENSION(npts), INTENT(inout)             :: Tseason
    ! temporary variable to calculate Tseason 
    REAL(r_std), DIMENSION(npts), INTENT(inout)             :: Tseason_tmp
    ! temporary variable to calculate Tseason: number of days when t2m_week is higher than 0 degree 
    REAL(r_std), DIMENSION(npts), INTENT(inout)             :: Tseason_length
    
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)         :: Tmin_spring
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)         :: Tmin_spring_time
    REAL(r_std), DIMENSION(npts,nvm,2), INTENT(inout)         :: onset_date
...
!ENDJCADD
    ...
    REAL(r_std), DIMENSION(npts)                            :: solai                    !! maximal radation during current day
                                                                                        !! (clear sky condition)
    REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:)            :: cloud                    !! cloud fraction
!$OMP THREADPRIVATE(cloud)
...
!_ =================================================================================================================================
    ...
    IF ( firstcall ) THEN
       ...
       !! 1.3.2.1 "long term" (::t2m_longterm)
...
!ENDJCADD
    ...
    !!    temperatures are given by the parameters ::tau_longterm, 
    ...
    !!    These variables are used in many different modules of STOMATE. The 
    !!    "long-term" temperature is used to calculate the long-term reference
    !!    temperature which is used in many modules as described below. 
    ...
    !
    !! 3.2 "long-term reference" (::tlong_ref) 
    !!      This variable is calculated using the "long term" 2-meter temperature 
    !!      (::t2m_longerm) using the equation:
    !!      \latexonly
    !!      \input{season_t2m_long_ref_eqn3.tex}
    !!      \endlatexonly
    !!      \n
    !!      This temperature is used for re-calculating PFT-specific variables such as
    ...
    !!      static, i.e. if the LPJ-DVM is used. It is also used in many STOMATE modules,
    !!      including ::stomate_npp, ::stomate_resp, ::stomate_phenology, 
...
!!$    ENDIF
    ...
    !
    ! record the onset date
    onset_date(:,:,1)=onset_date(:,:,1)+1
    WHERE ( begin_leaves(:,:) )
       onset_date(:,:,2)=onset_date(:,:,1)
    ELSEWHERE 
       onset_date(:,:,2)=0
    ENDWHERE
    !
